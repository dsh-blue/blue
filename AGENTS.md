...
| transcript | `@dsh-blue/blue-transcript` | session events → transcript items and rendering | fold · tool cards/read groups · `blueStatus` + footer shell + 5 status plugins · dock panes (activity/todo/btw) · `blueIntents` + `./intent-diff`/`./intent-terminal` · window/step folding · `./banner` | [AGENTS.md](packages/transcript/AGENTS.md) |
| interaction | `@dsh-blue/blue-interaction` | input editor, slash commands, dialogs | commands + alias registry · dialog panels (D30 editor-slot replacement) · completions (slash/`@`) · model/session-info/export/tools/preset/permission/plan-review families · shared editor seams · `./attachments`/`./paste-image`/`./pane-queue`/`./editor-plus` | [AGENTS.md](packages/interaction/AGENTS.md) |
