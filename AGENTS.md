...
**Blue** is the interactive terminal UI for [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) (`dsh`). It is a renderer over the harness's [Cordis](https://github.com/deepseek-ai/cordis) plugin architecture, built on `@earendil-works/pi-tui`, and shipped as six `@dsh-blue/blue-*` packages (all at version `0.1.0-rc.4` — the preview line, the number the website's tagline promises; blue-api first published with rc.3). The packages were extracted from the `deepseek-harness` monorepo; this standalone repository builds and tests against the published npm releases of the harness and vendored Cordis. Blue is **not** part of a default `dsh` installation — it is added to a `dsh` profile as an out-of-tree plugin bundle.
...
- There are TWO version lines under global control (`packages/transcript/tests/version.spec.ts` fails on any drift). **Blue's release line**: the seven `package.json` versions (six publishable packages + `website/`), `BLUE_VERSION`, and the website's user-facing copy (`index.md` tagline, guide/faq) must all equal `0.1.0-rc.4` — the preview line, which the site advertises. **The harness line**: every exact-pinned `@deepseek-ai/dsh-*` dev dependency, every `^`-ranged dsh peer, the `pnpm-workspace.yaml` `minimumReleaseAgeExclude` pins, and the `/version` notice's `HARNESS_LINE` constant (`session-commands.ts`) must all agree with each other — they ride the harness prerelease line and are deliberately NOT tied to Blue's number. Bumping Blue touches the seven manifests + `BLUE_VERSION` + the website copy; upgrading the harness line touches the dsh pins + `HARNESS_LINE`.
...
The README hero GIF, the still screenshots, and the og/social cards are generated, never hand-drawn:
...
- **`pnpm logo:export`** (`script/export-logo.mjs`) — derives `website/public/og.png` (1200×630) and `docs/assets/social-preview.png` (1280×640) from the interim logo: the blue terminal favicon. The whale banner is product UI only — promo visuals never use it (DeepSeek mascot association, 2026-08 ruling).
