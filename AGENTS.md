...
- There are TWO version lines under global control (`packages/transcript/tests/version.spec.ts` fails on any drift). **Blue's release line**: the eleven release package manifests plus `website/`, both `BLUE_VERSION` constants, and website copy equal `0.1.0-rc.10`. Validation-only package versions are outside this lockstep set; in particular `packages/context/package.json` remains `0.1.0-rc.2`. **The harness line**: exact dsh runtime/dev pins, `^` peers, workspace release-age excludes, and `/version`'s `HARNESS_LINE` all agree on their independent Harness prerelease line.
