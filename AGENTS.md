...
- **interaction** — bottom input editor (`src/input-plugin.ts`, the pi-tui Editor behind `ctx.blueComponents.createEditor` plus a separate hint-line component), slash commands `/quit` and `/resume` (`src/commands-plugin.ts`) and `/theme` (`src/theme-switch.ts`, hot-swaps the `blueTheme` provider via `ctx.registry.delete` + `ctx.plugin`, restoring dark on mount failure), user-question and approval overlays (`src/questions-plugin.ts`, `src/approval-plugin.ts`). The optional `./editor-plus` subpath plugin (`src/editor-plus.ts`, `blue-editor-plus`) layers `!` bash mode and slash/`@` autocomplete over the shared editor. Overlay key handling resolves through `ctx.blueKeymap`; text-editing keys are owned by the pi-tui Editor; the main editor's contextual keys (Escape/Ctrl-C/Ctrl-S) resolve through the editor's `onKey` pre-dispatch hook. A module-level draft stash (`src/draft-stash.ts`) mirrors the unsubmitted editor text so it survives theme-swap reloads (restored via `setText` on apply; cleared on submit/steer). Key discipline: a keymap action without a `handler` is contextual (components resolve it via `matches`), a handler-carrying action is global and is consumed by core's dispatcher before focus routing.
...
- **bundle/blue** — the installable unit. Its `cordis.patch.yml` inserts the Blue plugin rows over `dsh-base` in two commented segments: the plain baseline (seven rows: `blue-core` + `blue-theme-dark` + `blue-transcript` + `blue-status-basic` + `blue-interaction` + `blue-startup` + `blue-app`) and the enhancement segment (three rows: `blue-editor-plus` + `blue-status-git` + `blue-status-context`) — ten rows total; the bundle module itself mounts nothing. The four library packages are its `workspace:^` dependencies.
...
As of this writing, `pnpm run test` (320 tests, 34 files), `pnpm run typecheck`, and `pnpm run lint` all pass on Node 22+/pnpm 11.
