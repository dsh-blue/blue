...
| transcript | `@dsh-blue/blue-transcript` | semantic/legacy transcript rendering | official conversation model · legacy fold fallback · tool cards/read groups · `blueStatus` + footer shell + 5 status plugins · dock panes (activity/todo/btw/agents) · `blueIntents` + `./intent-diff`/`./intent-terminal` · window/step folding · `./banner` | [AGENTS.md](packages/transcript/AGENTS.md) |
...
| bundle/blue | `@dsh-blue/blue` | installable unit | `cordis.patch.yml` (three segments, 29 Blue rows + thin-host roster/disables) · `bundle.spec.ts` drift guard | [AGENTS.md](packages/bundle/blue/AGENTS.md) |
