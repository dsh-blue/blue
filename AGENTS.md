...
**Blue** is the interactive terminal UI for [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) (`dsh`). It is a renderer over the harness's [Cordis](https://github.com/deepseek-ai/cordis) plugin architecture, built on `@earendil-works/pi-tui`, and organized as thirteen `@dsh-blue/blue-*` packages (all at version `0.1.0-rc.2` — the preview line, the number the website's tagline promises). The packages were extracted from the `deepseek-harness` monorepo; this standalone repository builds and tests against the published npm releases of the harness and vendored Cordis. Blue is **not** part of a default `dsh` installation — it is added to a `dsh` profile as an out-of-tree plugin bundle.
...
| transcript | `@dsh-blue/blue-transcript` | semantic/legacy transcript rendering | official conversation model · legacy fold fallback · tool cards/read groups · `blueStatus` + footer shell + 5 status plugins · dock panes (activity/todo/btw/agents) · `blueIntents` + `./intent-diff`/`./intent-terminal`/`./intent-cordis` · window/step folding · `./banner` | [AGENTS.md](packages/transcript/AGENTS.md) |
...
| bundle/blue | `@dsh-blue/blue` | installable unit | `cordis.patch.yml` (three segments, 25 Blue rows + thin-host roster/disables + the S39 cordis host runner) · `bundle.spec.ts` drift guard | [AGENTS.md](packages/bundle/blue/AGENTS.md) |
| cli | `@dsh-blue/blue-cli` | the `blue` launcher shell — standalone global bin, outside the plugin tree | nested pinned `@deepseek-ai/dsh` host · profile calibration (`blue`, link-lane skip) · argv translation (`-V` self-answer, `plugin` subcommand, `--profile` swallow) · `BLUE_LAUNCHER` rebrand env · creative-preset overlay (S39: syncs `presets/cordis/` over the nested host's shipped `cordis` at boot) | [AGENTS.md](packages/cli/AGENTS.md) |

Legacy renderer direction remains core ← transcript / interaction ← app ← bundle. The target runtime adds Harness domain → conversation/context → frontend/transcript, api/frontend ← harness-adapter/remote, and optional OpenPencil/Lark interaction adapters; only core crosses into pi-tui. `@deepseek-ai/cordis` and the `dsh-*` service packages are `peerDependencies` (provided by the host `dsh` installation), mirrored as pinned `devDependencies` for local builds and tests. Core additionally carries `@deepseek-ai/schemastery` as a real runtime dependency (theme-custom config validation) and `cli-highlight` (code-fence syntax coloring behind the markdown `highlightCode` hook), alongside `@earendil-works/pi-tui`.
...
- `script/install-dev.sh` builds and link-installs the bundle plus all twelve library packages into a `dsh` profile (overrides: `DSH_BIN`, `PROFILE`, `DSH_HOME`).
...
- There are TWO version lines under global control (`packages/transcript/tests/version.spec.ts` fails on any drift). **Blue's release line**: the fourteen `package.json` versions (thirteen publishable packages + `website/`), `BLUE_VERSION`, and the website's user-facing copy (`index.md` tagline, guide/faq) must all equal `0.1.0-rc.2` — the preview line, which the site advertises. **The harness line**: every exact-pinned `@deepseek-ai/dsh-*` dev dependency, every `^`-ranged dsh peer, the `pnpm-workspace.yaml` `minimumReleaseAgeExclude` pins, and the `/version` notice's `HARNESS_LINE` constant (`session-commands.ts`) must all agree with each other — they ride the harness prerelease line and are deliberately NOT tied to Blue's number. Bumping Blue touches the fourteen manifests + `BLUE_VERSION` + the website copy; upgrading the harness line touches the dsh pins + `HARNESS_LINE`.
- `packages/bundle/blue` depends on all twelve libraries via `workspace:^`, which is unresolvable outside this workspace — relevant when installing into a `dsh` profile (link the complete closure, as `script/install-dev.sh` does).
