...
**Blue** is the interactive terminal UI for [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) (`dsh`). It is a renderer over the harness's [Cordis](https://github.com/deepseek-ai/cordis) plugin architecture, built on `@earendil-works/pi-tui`, and organized as twelve `@dsh-blue/blue-*` packages (all at version `0.1.0-rc.2` — the preview line, the number the website's tagline promises). The packages were extracted from the `deepseek-harness` monorepo; this standalone repository builds and tests against the published npm releases of the harness and vendored Cordis. Blue is **not** part of a default `dsh` installation — it is added to a `dsh` profile as an out-of-tree plugin bundle.
...
| bundle/blue | `@dsh-blue/blue` | installable unit | `cordis.patch.yml` (three segments, 27 Blue rows + thin-host roster/disables) · `bundle.spec.ts` drift guard | [AGENTS.md](packages/bundle/blue/AGENTS.md) |

Legacy renderer direction remains core ← transcript / interaction ← app ← bundle. The target runtime adds api/frontend ← harness-adapter/context/remote and optional OpenPencil/Lark interaction adapters; only core crosses into pi-tui. `@deepseek-ai/cordis` and the `dsh-*` service packages are `peerDependencies` (provided by the host `dsh` installation), mirrored as pinned `devDependencies` for local builds and tests. Core additionally carries `@deepseek-ai/schemastery` as a real runtime dependency (theme-custom config validation) and `cli-highlight` (code-fence syntax coloring behind the markdown `highlightCode` hook), alongside `@earendil-works/pi-tui`.
...
- `script/install-dev.sh` builds and link-installs the bundle plus all eleven library packages into a `dsh` profile (overrides: `DSH_BIN`, `PROFILE`, `DSH_HOME`).
