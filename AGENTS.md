...
- **bundle/blue** — the installable unit. Its `cordis.patch.yml` inserts the Blue plugin rows over `dsh-base` in two commented segments: the plain baseline (`blue-core` + `blue-theme-dark` + `blue-transcript` + `blue-interaction` + `blue-startup` + `blue-app`) and the enhancement segment (`blue-editor-plus`); the bundle module itself mounts nothing. The four library packages are its `workspace:^` dependencies.
...
As of this writing, `pnpm run test` (281 tests, 30 files), `pnpm run typecheck`, and `pnpm run lint` all pass on Node 22+/pnpm 11.
