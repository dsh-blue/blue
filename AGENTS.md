...
| transcript | `@dsh-blue/blue-transcript` | session events → transcript items and rendering | fold · tool cards/read groups · `blueStatus` + footer shell + 5 status plugins · dock panes (activity/todo/btw/agents) · `blueIntents` + `./intent-diff`/`./intent-terminal`/`./intent-cordis` · window/step folding · `./banner` | [AGENTS.md](packages/transcript/AGENTS.md) |
