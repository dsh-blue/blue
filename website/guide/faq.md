...
Blue 是装进 dsh profile 的插件包，不是独立应用——裸装只把包放进 node_modules，没有宿主与 profile 装配，跑不起来。正确路径：装 `blue` 壳包，或用 `dsh plugin --profile blue add`，见[快速上手](/guide/)。当前验收包为 `v0.1.0-rc.9-test.9`，发布在 **`rc9-test`**；生产 `rc` 与 `latest` 仍指向 rc.8。`0.1.0-rc.1` 因打包缺失文件不可用，装到请升级。贡献者的本地开发安装见开发手册的[贡献本仓库](/plugins/contributing)页。
...
- 立即装到新版：改用精确版本号——`dsh plugin --profile blue add @dsh-blue/blue@0.1.0-rc.9-test.9`（版本号以仓库最新 tag 为准）；
