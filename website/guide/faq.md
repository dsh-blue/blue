...
预览期每个通过真实 registry 安装验证的版本会同时推进 **`rc` 和 `latest`**，所以裸安装会落到同一个已验证版本；需要明确预览通道时可写 `@rc`。首个 stable 发布后，`latest` 才只指向稳定线。请装 `blue` 壳包 `npm i -g @dsh-blue/blue-cli@rc`，或用 `dsh plugin --profile blue add @dsh-blue/blue@rc`，见[快速上手](/guide/)。当前预览版为 `v0.1.0-rc.8`；`0.1.0-rc.1` 因打包缺失文件不可用，装到请升级。贡献者的本地开发安装见开发手册的[贡献本仓库](/plugins/contributing)页。
...
此外 Blue 启动后会后台检查一次新版（每 24h 至多查询一次 registry、失败静默、只读元数据、不做任何上报）。发现新版后，它会按当前 profile 的 pnpm `minimumReleaseAge` 计算可安装时间：冷却期内只把版本和时间记入本地状态，不显示提示；冷却结束后的下一次启动会直接从缓存把两行升级提示放到会话流顶部，不必再等 24 小时查询窗口。这个流程只通知，绝不会自动安装；升级仍需由你运行并确认 `/update`。不想要启动检查，在 `~/.dsh/settings.yaml` 写入：
