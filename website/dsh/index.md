...
::: info 版本基准
本手册当前以 npm 上的 `0.1.2-alpha.2` Harness 发布线为准。Blue `0.1.2-alpha.1` 不兼容 Harness RC；遇到出入以你安装的 `dsh --version` 与 `--dump-config` 为准。
:::
...
- **Blue** —— 就是一个 bundle：在 `dsh-base` 之上插入 34 条 Blue 自有行（含隔离管理权限与 raw app service 的 private runtime group），把终端交互界面接管过来（见[功能总览](/features/)）。
