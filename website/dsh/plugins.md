...
| **Code Mode 运行时** | `@deepseek-ai/dsh-code-runtime-worker-thread`（TypeScript）或 `@deepseek-ai/dsh-code-runtime-python` | `run_code` 的执行环境 | PTC 模式的前提：**默认无运行时**，`tools.mode: code/both` 必须先装其一 |
...
# Code Mode 运行时（TypeScript）：一行业务代码执行环境
- id: code-runtime
...
- **Code Mode 运行时是隔离非安全边界**：`worker-thread` 每次运行一个全新 worker（空环境、堆上限、硬终止）——遏制而非安全边界，信任姿态等同 bash；
...
- **lsp 工具** → 通用工具卡（暂无专属卡面）；
- **Code Mode**：PTC 模式在 Blue 中照常切换，但模型实际能用 `run_code` 之前，profile 必须装运行时——`--dump-config` 里没有 code-runtime 行就是没装；
