...
- **Revise** opens the inline feedback editor; submitting steers the feedback to the agent as a user message — the agent iterates on the plan with the notes;
- Approve/Reject settle directly; an aborted signal closes the panel with the cancellation code (`ASK_CANCELLED`).

Plan mode itself enters/exits through the `Shift+Tab` three-state cycle (see [Session modes](/en/features/modes)); the footer's mode badge reflects it live.

## Permission-preset panel

`/permission` opens the permission-preset selector (the same single-select list shape as `/sessions` and `/preset`): one row per preset (a named bundle of sandbox mode + approval policy), the active one checkmarked; Enter switches through the host's same write path, and a **danger-level preset requires a typed `y`** (no accidental switches). The command opens via input-layer interception and is not in the `/help` registry.
