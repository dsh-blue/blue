...
- **Public capabilities only**: the plugin uses only capabilities accepted by the current Beta host, and its declaration matches reality. Beta capabilities are `commands` / `status` / `notifications.publish` / `panes` / `overlays` / `session.read` / `session.projections.read`; `editor.extensions` / `status.provider` / `editor.provider` must be described explicitly as Experimental/reference (see the capability contracts in [Core concepts](/en/plugins/concepts));
