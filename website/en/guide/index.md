...
::: info Preview stage
`v0.1.0-rc.8` is published under npm's **`rc` and `latest` dist-tags**. Use `@rc` to state the preview channel explicitly; after the first stable release, `latest` points only to stable. This page is the user install path; the contributor development install (checkout, link install, iteration loop) lives in the developer manual under [Contributing to Blue](/en/plugins/contributing).
:::
