...
Preview releases are published only under the **`rc` dist-tag** (`latest` stays reserved for the stable line), and a bare install resolves `latest` — use `dsh --profile blue plugin add @dsh-blue/blue@rc` instead, see [Quickstart](/en/guide/). `v0.1.0-rc.1` is the first released version; the local development install (contributor path) lives on the same quickstart page.
