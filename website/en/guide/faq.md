...
Blue is a plugin bundle installed into a dsh profile, not a standalone app — a bare install only drops the package into node_modules, with no host and no profile assembly, so there is nothing to run. The supported paths are the `blue` shell or `dsh plugin --profile blue add`; see [Quickstart](/en/guide/). The current release candidate is `v0.1.0-rc.9`; npm's `rc` and `latest` tags point to this release. `0.1.0-rc.1` shipped broken tarballs (missing files) — if it is installed, upgrade. The contributor development install lives in the developer manual under [Contributing to Blue](/en/plugins/contributing).
...
- install the exact version right away — `dsh plugin --profile blue add @dsh-blue/blue@0.1.0-rc.9` (match the repository's newest tag);
