...
::: info
The user install path is npm — `dsh plugin --profile blue add @dsh-blue/blue@alpha`; see [Quickstart](/en/guide/). This page only serves contributors hacking on Blue itself.
:::
...
| Node | `^22.19.0 \|\| >=24.0.0` |
| pnpm | 11 |
| dsh CLI | exact `0.1.2-alpha.2` (RC is unsupported) |
...
- **`blue`** = the production profile, **npm installs only** (`@dsh-blue/blue@alpha` / an exact version). Never `link:` into it — a later npm upgrade overwrites only the named packages, the leftover links dangle, and boot dies with `ERR_MODULE_NOT_FOUND` (`pnpm add` does not warn about the mix).
