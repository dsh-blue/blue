...
| `/settings` | — | — | Open the two-level settings panel (pick a namespace, then preset-cycle values; the last row opens settings.yaml in `$EDITOR`) | `blue-commands` (via settings-command) |
