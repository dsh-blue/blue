# 插件市场

::: info 建设中
Blue 生态插件市场将以独立仓库实现（收录、索引与卡片展示），目前处于规划阶段。本页为占位，开放后同步上线。
:::

## 这里是什么

面向 **Blue 下游开发者**的生态插件市场：你用 [Seam](/plugins/seams) 编写的插件——状态栏条目、工具卡片、编辑器增强、全新主题、完整面板——发布到 npm 后收录于此，其他用户一行安装。

每张卡片大致长这样：

```
┌─────────────────────────────────────┐
│ my-plugin-clock              v0.1.0 │
│ 状态栏时钟 + /now 命令                 │
│ ★ 12 · 状态栏 / 命令                  │
│ dsh plugin add my-scope/my-pkg      │
└─────────────────────────────────────┘
```

## 上线之前

- 想开发插件：从[编写第一个插件](/plugins/)开始，缝的目录见 [Seam 参考](/plugins/seams)；
- 想看插件能做什么：Blue 自带的 21 个插件就是活例子，见[内置插件](/plugins/builtins)；
- 关注 [GitHub 仓库](https://github.com/dsh-blue/blue)获取市场开放公告。
