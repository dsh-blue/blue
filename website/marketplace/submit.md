...
- **基于公开能力**：只使用当前开放的八种能力 `commands` / `status` / `notifications` / `panes` / `overlays` / `editor.extensions` / `status.provider` / `editor.provider`，且声明与实际相符（能力契约见[核心概念](/plugins/concepts)）；
