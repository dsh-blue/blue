...
仓库 `examples/` 提供一个共享用户 kit、七个可运行的 opt-in 插件，以及一个把
七行组合在一起的验证 bundle。它们是可打包发布的参考实现，但不进入 Blue 默认
bundle 或当前 release set。

::: warning 示例的契约层级
其中六个运行插件来自 PR #77；所有示例插件的 `blue.plugin.json` 仍走旧 flat transition lane；两个
...
| `@dsh-blue-example/ui-gallery` | `panes` | right lane 静态展示公共 UI builder；tab strip 分组，窄屏降级 bottom |
...
`@dsh-blue-example/blue-ecosystem` 的 patch 按上表顺序装配七行；它不是第九个
运行场景。
...
也可以安装 composition bundle，一次激活七个插件。先确保同一 profile 已安装
...
门禁先对九个 package 运行兼容 validator，再分别对当前与上一条 Harness 线：

- `pnpm pack` API、UI、core、kit、七个插件与 composition；
- 安装进一个无 workspace link 的临时 npm 项目；
- 只从安装后的 public exports 导入；
- 校验 transition manifest、单行 patch、七行 composition、无 `src/` 与本地协议泄漏；
- 执行九个场景和 20/40/80/120 列宽度扫描；
- 要求 `declared === executed === 9`、零 skipped/failure 且临时目录已清理。
