...
- `@dsh-blue/blue-api@0.1.1-rc.3`（用到 builder 时再加 `@dsh-blue/blue-ui@0.1.1-rc.3`）在 `dependencies`，`@deepseek-ai/cordis` 在 `peerDependencies`——后者由宿主 dsh 提供，打进 `dependencies` 会出现第二份服务实例；
...
与详情路由，并暂停收录；旧 `verified` 不是 rc.3 兼容性或 conformance 证据。P5 的
