...
`/permission` 打开权限预设选择器（与 `/sessions` `/preset` 同款单选列表面板）：每行一个预设名（sandbox 模式 + 审批策略的命名束），当前预设带 `← current` 标记；Enter 切换走宿主同一写路径，**danger 级预设需打字 `y` 确认**（防误触）。命令经输入层拦截开面，不在 `/help` 注册表。
