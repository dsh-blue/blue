...
`./locale` adapter 把 Harness 的 `locale.preference` setting 绑定到单棵 frontend tree 的 `blueLocale` 服务。没有显式偏好时依次跟随 `LC_ALL`、`LC_MESSAGES`、`LANG` 与 `Intl`；中文变体统一使用简体中文，不支持的语言回退英文。
