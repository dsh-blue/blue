...
The dsh Blue bundle: the interactive terminal UI profile. [`cordis.patch.yml`](cordis.patch.yml) rides directly over [`dsh-base`](https://github.com/deepseek-ai/deepseek-harness) and inserts 31 Blue-owned rows: two host-support rows plus 29 product rows in baseline, enhancement, and assembly segments. The projection-backed `blue-conversation` and `blue-transcript-official` rows are part of the self-contained baseline; the 15 enhancement rows are individually removable. `blue-context`, `blue-remote`, `blue-openpencil`, and `blue-lark` remain validation-only packages outside the bundle dependency closure.

The bundle owns Blue's complete agent-preset roster. Standard, PTC, and minimal modes track the pinned harness line; `cordis` / 创造模式 carries the Blue-aware persona and capability-scoped plugin guidance. Both `blue` and direct `dsh --profile` launches resolve this immutable bundle payload, without rewriting the host's shared presets. Creative prototypes may add pane, status, command, and notification contributions through `bluePluginHost`; they may also register inert `status.provider` and `editor.provider` candidates, or request separately scoped `session.read` and `session.act` facades. Only the persisted `blue.statusProvider` and `blue.editorProvider` user choices can activate provider candidates. Prototypes cannot resolve Blue's owner registries or replace core feature IDs. A current compatible host accepts and durably buffers commands, status entries, panes, overlays, editor extensions, and provider candidates even while their frontend owner is absent; they remain inert until an owner replays them. `BLUE_CAPABILITY_ABSENT` instead means an old or incompatible host/profile lacks that durable buffer, or the active notifications/session owner is absent. After prototype acceptance, the Agent asks before keeping local code, creating a GitHub repository, or publishing an npm package.
