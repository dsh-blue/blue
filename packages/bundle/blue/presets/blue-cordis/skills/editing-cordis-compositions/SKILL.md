...
In Blue the host composition is `dsh-base` plus the Blue bundle's `cordis.patch.yml` (three segments: the thin-host disables, the baseline UI rows, the enhancement and assembly rows), and a profile composes MULTIPLE bundles in order — a third-party Blue feature is another bundle layered after Blue, not an edit to Blue's patch. A row that changes what the terminal *shows* does not belong in a preset at all — it belongs in such a Blue plugin package (see the blue-plugin-development skill). The `/preset` command switches a blank session's composition honestly; it cannot change the UI that renders the session.
