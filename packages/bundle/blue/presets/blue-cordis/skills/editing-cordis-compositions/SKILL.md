...
description: Use when creating, changing, or validating a Cordis composition for this harness — writing or editing an agent preset, adding or removing a plugin row, deciding whether something belongs to the host composition, to one session, or to Blue's own bundle patch, checking whether a preset you authored actually mounts, or diagnosing a row that mounted but contributed nothing.
---
...
**Never edit, delete, or overwrite a preset that ships with the deployment** — the upstream shipped root supplies `standard`, `ptc`, `minimal`, and `cordis`, while Blue's bundle-local system root supplies only `blue-cordis`. Never escalate the sandbox to reach either root, even when a change there looks quicker. An upgrade overwrites those installs, and corrupting a creative preset disables preset authoring itself. Reading a shipped composition is the intended way to start; writing to one is not, and neither is editing the host composition to work around a preset limitation.

In Blue there is one more off-limits subtlety: the `blue-cordis` preset you are running on is Blue's own creative mode, loaded from the immutable Blue bundle. A request to change that shipped preset is Blue repository maintenance, outside every preset author skill. Stop and direct the maintainer to the repository root `AGENTS.md` and `packages/bundle/blue/AGENTS.md`; do not mutate the installed preset or route the work through `blue-plugin-development`.
...
In Blue the host composition is `dsh-base` plus the Blue bundle's flat
`cordis.patch.yml`. Native dsh services, Blue rows, and third-party plugin
rows share one Cordis service graph. A profile composes multiple bundles in
order, so a third-party Blue feature is another ordinary bundle layered after
Blue, not an edit to Blue's patch. A row that changes what the terminal
*shows* does not belong in an agent preset; it belongs in a plugin package
(see `blue-plugin-development`). The `/preset` command switches a blank
session's agent composition; it cannot change the UI rendering that session.
...
After a clean mount-validation, ask the user to start a session on the new preset and confirm the tool list; the preset decides tool schemas and prompt sections, and only a real session shows the agent that composition produces. In Blue that means switching a blank session with `/preset <id>`.
