...
The installable unit. Its `cordis.patch.yml` inserts the Blue plugin rows over `dsh-base` in three commented segments — the baseline segment (five rows: `blue-core` + `blue-theme-dark` + `blue-banner` + `blue-transcript` + `blue-status-basic`; the banner sits before the transcript so the shared `blueComponents` activation round keeps it the first scroll child across `/theme` reloads), the enhancement segment (sixteen rows: `blue-editor-plus` + `blue-attachments` + `blue-paste-image` + `blue-status-cwd` + `blue-status-git` + `blue-status-title` + `blue-status-mode` + `blue-status-tips` + `blue-status-context` + `blue-intent-diff` + `blue-intent-terminal` + `blue-pane-activity` + `blue-pane-queue` + `blue-pane-todo` + `blue-pane-btw` + `blue-pane-agents`), and the assembly segment closing the plain baseline (three rows: `blue-interaction` + `blue-startup` + `blue-app`) — twenty-four Blue rows total, headed by two host-plane rows: the upstream `agent-presets` roster (D37) and the `session-title-all-prompts-llm` cadence row (S30). The bundle module itself mounts nothing. The four library packages are its `workspace:^` dependencies.
