...
The `blue-plugin-session-bridge` assembly row follows `blue-app` and explicitly injects `bluePluginControl` plus `blueSessionReader`. The control and raw reader are available only inside `blue-runtime-private`, so public `session.read` cannot become ready before both the API host and app-owned reader exist. Creative child Fibers inherit only `bluePluginHost`; control, reader, projections, and broad actions remain isolated. Generic public `session.act` is absent.
