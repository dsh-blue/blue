...
F5 carries disabled `blue-conversation` and `blue-transcript-official` rows. The domain row registers the official append-origin `blueConversation` projection and then publishes its effect-scoped readiness capability. The consumer injects that capability, so concurrent sibling activation cannot snapshot a resumed session before replay exists. While the official model is mounted, `blue-transcript` removes its legacy event-folded producer; unloading either acceptance row restores the existing fallback without a duplicate transcript. Both rows remain off in production until the dedicated profile receives live acceptance.
