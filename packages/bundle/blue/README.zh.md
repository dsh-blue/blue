...
dsh Blue 组合包：交互式终端 UI profile。[`cordis.patch.yml`](cordis.patch.yml) 直接叠加在 [`dsh-base`](https://github.com/deepseek-ai/deepseek-harness) 之上，共插入 29 条 Blue 自有行：2 条宿主支撑行，加上分为基线、增强、装配三段的 27 条产品行。基线包含用于系统语言探测与中英文实时切换的 `blue-locale`，以及 projection-backed 的 `blue-conversation` 与 `blue-transcript-official`；14 条增强行可逐项移除。`blue-context`、`blue-remote`、`blue-openpencil` 与 `blue-lark` 保持为 bundle 之外的 validation-only 包。
