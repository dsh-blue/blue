...
function fixture(initial = values(), initialSeq = 8) {
  ...
  const service: OfficialSessionProjectionService = {
    snapshot: vi.fn(() => ({ asOfSeq: seq, values: state })),
    onChanged: vi.fn(listener => { changed = listener; return off }),
  }
  return {
    session,
    other,
    ...
    emit(target: unknown, key: string, eventSeq: number) { changed?.(target, key, state[key], eventSeq) },
  }
}
...
describe('OfficialContextSource', () => {
  it('closes the baseline/subscribe gap and coalesces same-seq key changes', async () => {
    const f = fixture()
    const source = new OfficialContextSource(f.service, id => id === 's1' ? f.session : undefined)
    ...
    f.emit(f.session, 'contextPressure', 9)
    f.emit(f.session, 'tokenUsage', 9)
    ...
    expect(f.service.snapshot).toHaveBeenCalledTimes(2)

    f.set(values(), 11)
    f.emit(f.session, 'sessionStats', 10)
    f.emit(f.session, 'contextTimeline', 10)
    f.emit(f.session, 'contextBreakdown', 11)
    await Promise.resolve()
    ...
    off()
    f.set(values(), 12)
    f.emit(f.session, 'tokenUsage', 12)
    await Promise.resolve()
    f.set(values(), 13)
    f.emit(f.session, 'tokenUsage', 13)
    await Promise.resolve()
    f.set(values(), 11)
    f.emit(f.session, 'tokenUsage', 11)
    await Promise.resolve()
  ...
  it('filters listeners by watermark, ignores unknown sessions, and drops late work after dispose', async () => {
    const f = fixture()
    const source = new OfficialContextSource(f.service, id => id === 's1' ? f.session : undefined)
    ...
    f.set(values(), 10)
    f.emit(f.other, 'tokenUsage', 10)
    f.emit(f.session, 'tokenUsage', 10)
    await Promise.resolve()
