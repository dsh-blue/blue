...
describe('context projection', () => {
  ...
  it('builds timeline sections and omits refresh for push-only official projections', () => {
    ...
    expect(model.status.nodes[0]).toMatchObject({ content: 'context · 1 requests' })
    ...
    expect(sparse.status.nodes[0]).toMatchObject({ content: 'context · 0 requests' })
