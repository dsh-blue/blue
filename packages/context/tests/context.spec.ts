...
describe('context projection', () => {
  ...
  it('builds timeline sections and omits refresh for push-only official projections', () => {
    ...
    expect(model.panel.submit).toBeUndefined()
    ...
    expect(model.panel.view).toMatchObject({ kind: 'sections' })
    ...
    expect(noProvider.panel.view).toMatchObject({ kind: 'sections' })
    ...
    expect(fallbackEvents.panel.view).toMatchObject({ kind: 'sections' })
  })
})
