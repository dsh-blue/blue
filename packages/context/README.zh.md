...
model 包含 provider usage、context occupancy、composition、当前 surface 分类以及最近的 request/context event。projection key 可独立卸载；下一次官方切片会移除对应 section 和 capability。兼容 source 仍可提供结构化 refresh action。该包不依赖 terminal 或 renderer。
