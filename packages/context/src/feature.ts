...
import type { ContextAction, ContextModel, ContextModelState, ContextSnapshot, ContextSource, ContextState } from './types.ts'
export class ContextFeature {
  ...
  constructor(source?: ContextSource) { this.projection = new ContextProjection(source); this.canRefresh = source?.refresh !== undefined && (source.capabilities === undefined || source.capabilities.includes('refresh')); this.projection.subscribe(snapshot => this.emit(buildContextModel(snapshot, this.state ?? 'ready', this.error, this.canRefresh))) }
  get snapshot(): ContextSnapshot | undefined { return this.projection.snapshot }
  get model(): ContextModel | undefined { const snapshot = this.snapshot; if (snapshot !== undefined) return buildContextModel(snapshot, this.state ?? 'ready', this.error, this.canRefresh); if (this.state === undefined) return undefined; return buildContextModel({ sessionId: '', watermark: -1, facts: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 } }, this.state, this.error, this.canRefresh) }
  ...
  async execute(action: ContextAction): Promise<BlueResult<void>> {
    ...
    if (result.ok) { const snapshot = this.snapshot; this.state = snapshot === undefined || emptyFacts(snapshot.facts) ? 'empty' : 'ready'; this.emit(this.model); return { ok: true, value: undefined } }
...
function emptyFacts(facts: ContextSnapshot['facts']): boolean {
  return facts.input === 0 && facts.output === 0 && facts.cacheRead === 0 && facts.cacheWrite === 0 && facts.used === undefined && facts.breakdown === undefined && facts.timeline === undefined
}

function emptyState(state: ContextState): boolean {
  return emptyFacts(state.usage)
}
