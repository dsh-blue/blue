...
describe('registerTraceCommand', () => {
  ...
  it('opens the editor-slot panel and loads selected details', async () => {
    ...
    panel?.handleInput('\r')
    await vi.waitFor(() => { expect(notices).toContain('copied trace item #0') })
    ...
    options.onAction({ kind: 'trace.copy-all' })
    await vi.waitFor(() => { expect(notices).toContain('copied 1 trace events') })
    setClipboardTextWriter(async () => { throw new Error('native copy all unavailable') })
    setClipboardOsc52Emitter(() => true)
    options.onAction({ kind: 'trace.copy-all' })
    await vi.waitFor(() => { expect(notices.some(notice => notice.includes('copied 1 trace events via terminal escape sequence'))).toBe(true) })
    setClipboardTextWriter(async () => { throw new Error('copy all unavailable') })
    setClipboardOsc52Emitter(() => false)
    options.onAction({ kind: 'trace.copy-all' })
    await vi.waitFor(() => { expect(notices).toContain('could not copy trace: copy all unavailable') })
    setClipboardTextWriter(async () => { throw new Error('native clipboard unavailable') })
    setClipboardOsc52Emitter(() => true)
    options.onAction({ kind: 'trace.copy', seq: 0 })
    await vi.waitFor(() => { expect(notices.some(notice => notice.includes('terminal escape sequence'))).toBe(true) })
    setClipboardTextWriter(async () => { throw new Error('clipboard down') })
    setClipboardOsc52Emitter(() => false)
    options.onAction({ kind: 'trace.copy', seq: 0 })
    await vi.waitFor(() => { expect(notices).toContain('could not copy trace item: clipboard down') })
