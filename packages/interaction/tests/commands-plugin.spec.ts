...
describe('blue-commands plugin', () => {
  ...
  it('/help lists the registered commands and key bindings in an overlay', async () => {
    ...
    // 34 rows since S28 added /tools and /preset to the command list (32 at S27').
    expect(rows.some(row => row.includes('_ showing 1-16 of 34_'))).toBe(true)
    ...
    expect(scrolled.some(row => row.includes('_ showing 11-26 of 34_'))).toBe(true)
  ...
  it('/help falls back to the action id when a binding has no description', async () => {
    ...
    // downs clamp at the scroll floor (19 clears the S28-extended list).
    for (let i = 0; i < 19; i += 1) overlay(screen).handleInput(KEY.down)
    const rows = screen.overlays[0]?.component.render(80) ?? []
