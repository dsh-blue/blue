...
export const CHANGELOG_ENTRIES: readonly ChangelogEntry[] = [
  {
    ...
    highlights: [
      ...
      'Focus-derived operation hints — plugin surfaces and built-in panels now derive one localized hint row from the same control/edit/confirmation state that handles input. Tab switches semantic groups, arrows stay within the active group, narrow layouts remove complete hint fragments, and generic shortcut footers or passive Activity keyboard teaching no longer duplicate context-sensitive guidance.',
    ],
    knownIssues: [
  ...
  },
  {
    ...
    highlights: [
      ...
      'Two-tab /plugin management — Installed inventories current-profile packages with Verify/Remove actions; Catalog opens from a vetted snapshot, refreshes an explicit GitHub index in the background, survives offline, and exposes Details plus a commit-pinned Install action only for canonical compatible manifests. The vetted dsh-blue/blue-doudizhu@0.3.0 snapshot is canonical and installable at its full commit; pnpm receives the pin through its #commit delimiter. Direct local/npm/GitHub mutations still delegate to the profile owner with an explicit restart boundary.',
    ...
    knownIssues: [
      'The Website Marketplace remains paused — the built-in TUI Catalog is a bounded metadata index, not a marketplace unlock. Website builds still remove registry data and detail routes, and submissions/cards stay disabled until the separate P6 ecosystem and later marketplace migration gates close.',
