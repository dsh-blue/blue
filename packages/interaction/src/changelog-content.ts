...
export const CHANGELOG_ENTRIES: readonly ChangelogEntry[] = [
  {
    ...
    highlights: [
      ...
      'Two-tab /plugin management — Installed inventories current-profile packages with Verify/Remove actions; Catalog opens from a vetted snapshot, refreshes an explicit GitHub index in the background, survives offline, and exposes Details plus a commit-pinned Install action only for canonical compatible manifests. Legacy dsh-blue/blue-doudizhu is indexed as Needs migration with Install disabled. Direct local/npm/GitHub mutations still delegate to the profile owner with an explicit restart boundary.',
