...
Blue 自有 interaction chrome 支持英文与简体中文。`/settings` 首行显示“语言”，通过 Harness 的 `locale.preference` 在“跟随系统”“中文”“English”之间循环；切换时会刷新已打开的 settings/help/command surface，不替换 panel、不丢选择，也不丢弃已打开 form 的草稿。用户/model/tool 内容、路径、id、命令名与上游错误详情不翻译。
