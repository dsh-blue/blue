...
- **`blue-commands`** —— 注册 `/quit`（经启动器持有的 `ctx.appExit` 请求退出；启动器未提供时返回错误结果）、`/resume <session-id>`（emit `blue/request-resume`；真正的 resume 由 app 层执行）与 `/theme`（切换当前主题 provider：`/theme` 列出已知主题并标出当前主题，`/theme dark|light|auto` 切到内置调色板，`/theme custom <path> [dark|light]` 挂载文件调色板；切换经 `ctx.registry.delete` 释放当前 provider 的全部 fiber 后挂载替换者，挂载失败时回退到内置 dark 调色板）。
