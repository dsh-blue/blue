/** Blue renderer for the direct pane and overlay Cordis registries. */
import type { Context } from '@deepseek-ai/cordis'
import {
  type BlueOverlayEntry,
  type BluePaneEntry,
  type BlueUiEvent,
...
interface SurfaceSnapshot {
  ...
  readonly panes: readonly BluePaneEntry[]
  readonly overlays: readonly BlueOverlayEntry[]
}
...
function overlayAnchor(anchor: BlueOverlayEntry['request']['anchor']) {
  switch (anchor) {
...
export function mountBlueSurfaceRenderer(ctx: OwnerContext, runtime: BlueTerminalRuntime, translateHint?: (key: string) => string): void {
  ...
  const overlayViewport = (entry: BlueOverlayEntry): BlueUiViewport => {
  ...
  const addPane = (entry: BluePaneEntry): void => {
    let record!: PaneRecord
    const events = new SurfaceEventOwner(entry.id, entry.contribution.onEvent, () => schedulePane(record, 'internal'), undefined)
  ...
  const addOverlay = (entry: BlueOverlayEntry): void => {
    let record!: OverlayRecord
    const events = new SurfaceEventOwner(entry.id, entry.request.onEvent, () => scheduleOverlay(record, 'internal'), () => {
      ctx.blueOverlays.close(record.entry.id)
    })
    ...
    const compiled = compile(entry.request.render, 'overlay', {
      ...
      ...(entry.request.capturing && entry.request.dismissible !== false ? { onEscape: () => events.emit({ kind: 'dismiss' as const }), escapeHint: 'close' as const } : {}),
      ...(translateHint === undefined ? {} : { translateHint }),
      interactive: entry.request.capturing === true,
  ...
  const renderOverlay = (record: OverlayRecord, refreshMode: 'internal' | 'external'): void => {
    ...
    const compiled = compile(entry.request.render, 'overlay', {
      ...
      mode: runtime.mode,
      emit: event => record.events.emit(event),
      ...(entry.request.capturing && entry.request.dismissible !== false ? { onEscape: () => record.events.emit({ kind: 'dismiss' as const }), escapeHint: 'close' as const } : {}),
      ...(translateHint === undefined ? {} : { translateHint }),
      interactive: entry.request.capturing === true,
      runtime: record.runtime,
      refreshMode,
  ...
  let paneEntries = ctx.bluePanes.list()
  let overlayEntries = ctx.blueOverlays.list()
  let registryRevision = 0
  const publish = (): void => schedule({ revision: ++registryRevision, panes: paneEntries, overlays: overlayEntries })
  const offPanes = ctx.bluePanes.subscribe(entries => { paneEntries = entries; publish() })
  const offOverlays = ctx.blueOverlays.subscribe(entries => { overlayEntries = entries; publish() })
  ctx.effect(() => () => {
    disposed = true
    offPanes()
    offOverlays()
