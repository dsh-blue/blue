...
`compileBlueUiNode(value, { components, colors, getViewport, screenMode, emit })` 必定先经过上述 validator，再返回 canonical node、pi-tui 支撑的 component，以及至多一个 composite focus target。该 composite 独占 roving focus、实时响应式可见性协调和 event/render 异常隔离；单个控件不会作为 focusable 泄漏。焦点按外层 tabs、内层 tabs、内容控件组、编辑态逐层下钻。Tab 条使用不循环的 Left/Right，Enter 下钻，Tab 在 Tab 条上无动作；进入内容后，Tab/Shift-Tab 才按树序循环语义控件组，并恢复每组上次活动项。方向移动不循环：Left/Right 留在水平控件，Up/Down 留在 list 与 form。Escape 每次返回一层，最后才关闭 surface。聚焦的 action token 会获得紧凑的矩形选择背景，且不改变可见宽度；primary 方括号与 danger 标记仍表示 intent，而不是焦点。Select 先用 Enter 进入显式的 `‹ value ›` 调整态；Left/Right 修改本地候选，Enter 或合法值上的 Tab 确认，Escape 取消，非法值保持当前编辑态。文本字段分为导航态和编辑态：选中时只显示字段行焦点，Enter 或可打印/粘贴输入进入编辑；Enter 或合法值上的 Tab 确认，textarea 用 Alt+Enter 插入换行，非法字段保持活动，Escape 返回导航态。多选列表用 Space 切换、Enter 确认，disabled 行会被跳过。因此编辑态之外不显示反色 editor caret。公共文本无法注入 pi-tui cursor marker 或 core 私有 focus sentinel。Component 会暴露真实的 pi-tui layout node，使嵌套 stack/scroll 获得实际分配高度。Direct render（包括 AltScreen stop replay）使用私有 sentinel，在完整合成后替换；AltScreen layout pass 中 pi-tui 会刻意绕过 wrapper `render`，因此 active leaf 在 composite focus 协调后使用等宽 cursor-marker adapter。两条路径均保证聚焦时恰好一个 marker、失焦时没有 marker，并由真实 layout-frame 测试锁定 HStack 文本完整性。

聚焦的插件 pane/overlay 与官方 interaction panel 会从同一份控件状态追加一行上下文按键提示，不再各自维护快捷键 footer。提示会随导航、文本/select 编辑与二次确认切换；只在真实存在关闭/退出动作时显示 Escape；最多保留三个语义片段；窄屏先退化为完整的紧凑 token，再整段隐藏。被动 surface、status/editor shell 与未聚焦 surface 不显示该行。Blue 自有的中英文操作词会随 locale 原地切换。
...
私有 plugin-surface bridge 持有 live `panes`/`overlays` generation，并为每个已挂载 registration 持有一个 renderer runtime。事件成功触发的内部重绘会保留语义焦点、editor identity/cursor、本地 form draft 与待确认状态，即使 canonical tree 调整了顺序；有界缓存也允许最近暂时离树的 form field 在返回时恢复 draft 与 editor 状态，适配只渲染 active tab 的 projection，而 field kind 改变会启用全新状态。显式外部 refresh 则接受新的 canonical field value，但继续保留语义焦点。响应式隐藏只会临时回退，控件恢复可见后会回到同一语义控件；真正删除或禁用控件后，回退会成为新的稳定焦点。稳定的 pane/overlay wrapper 会 fence 旧 compiled generation，失败或非法 render 只显示有界 danger-node fallback，不会覆盖最后一个正常 runtime。Pane definition 会在 renderer reload 后从 API host replay，overlay open 则保持 transient：close、registration 替换、owner 替换或卸载都会先 dispose runtime，旧 callback 因此不能再发事件。Value/selection/tab event 采用 latest-wins，activate/submit/dismiss 采用 FIFO；timeout、abort、refresh、semantic close 与迟到 settlement 都由捕获的 owner generation 约束。
