...
`compileBlueUiNode(value, { components, colors, getViewport, screenMode, emit })` 必定先经过上述 validator，再返回 canonical node、pi-tui 支撑的 component，以及至多一个 composite focus target。该 composite 独占 roving focus、实时响应式可见性协调和 event/render 异常隔离；单个控件不会作为 focusable 泄漏。Tab/Shift-Tab 按树序在语义控件组之间移动，并恢复每组上次活动项；Left/Right 在 tabs 与 actions 组内移动，Up/Down 在 list 与 form 组内移动。Select 用 Left/Right 切换选项，把 Up/Down 留给 form 导航。文本字段分为导航态和编辑态：选中时只显示字段行焦点，Enter 或可打印/粘贴输入进入编辑，Escape 返回导航；Tab 先结束编辑，并在存在其它组时继续跨组。因此只有真正编辑时才显示反色 editor caret。公共文本无法注入 pi-tui cursor marker 或 core 私有 focus sentinel。Component 会暴露真实的 pi-tui layout node，使嵌套 stack/scroll 获得实际分配高度。Direct render（包括 AltScreen stop replay）使用私有 sentinel，在完整合成后替换；AltScreen layout pass 中 pi-tui 会刻意绕过 wrapper `render`，因此 active leaf 在 composite focus 协调后使用等宽 cursor-marker adapter。两条路径均保证聚焦时恰好一个 marker、失焦时没有 marker，并由真实 layout-frame 测试锁定 HStack 文本完整性。
