...
`./chrome` 子路径（`src/chrome.ts`，S11 开出）导出绘制圆角框表面的主题无关纯函数：`withSideBorders(lines, paint, {connectedAbove?, label?})` 把 pi-tui 风格的横线行变成 `╭╮`/`╰╯` 圆角框（预着色的横线剥 SGR 重涂为一个跨度、`│` 只叠在字面空格外列上以保护反显光标、标签只嵌入纯 `─` 串），`injectPromptSymbol(line, symbol, paint?)` 在 `paddingX: 4` 的内容行上叠提示符。色函数由调用方注入——模块不持有主题也不含 pi-tui 组件机制；其余函数各随首个消费者落地（S12 对话框、S13 面板、S14 补全）。
