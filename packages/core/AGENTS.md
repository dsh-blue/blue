...
`src/ui-compiler.ts` is the sole canonical node -> pi-tui compiler. `compileBlueUiNode` must call the ordinary validator itself; `compileBlueStatusNode` must independently call the recursively narrowed status validator, then both use the same admitted-node compiler and painter graph. The status result is a passive facade with no focus/input/event surface: row stacks retain compact spatial `HStack` semantics in both screen modes, output is runtime-bounded to 1..3 rows, and `renderStatus` reports overflow so the footer can choose hide versus truncate. The UI compiler's private `maxLeafRows` option exists only for bounded official compatibility adapters; public plugin surfaces omit it and retain the 20-row leaf default. Its returned composite is the only `BlueFocusable`: it owns roving state, live viewport reconciliation, event containment, and exactly-one cursor-marker insertion. It must expose the real root `LAYOUT_NODE`; an opaque wrapper leaves nested ScrollView state at zero. Direct render/stop replay uses a sentinel replaced after composition. AltScreen layout bypasses wrapper render, so its explicit layout-pass adapter emits an equal-width marker from the reconciled active leaf; real `renderLayoutFrame` HStack and replay tests must cover both paths. AltScreen scroll is non-primary/contained and clips to its stack-allocated height. Ordinary MainScreen UI unwraps scroll, linearizes row stacks, and preserves all document rows. Do not clamp stack sizing to the compile-time viewport; the fixed safety ceiling exists solely to bound hostile safe integers during later live resize.
...
The legacy frontend `View` bridge in `src/frontend-renderer.ts` is conversion-only: it maps the readonly frontend shapes to canonical public `BlueUiNode` wire data and invokes `compileBlueUiNode`, including validation and the same private pattern path as plugin contributions. `renderFrontendView`, `renderFrontendModel`, and `FrontendModelComponent` remain compatibility exports while interaction/transcript still publish the legacy model; delete the adapter with that model contract rather than adding another painter. Its passive list output intentionally adopts the canonical list furniture. Diff alignment, semantic paint, sanitation, and width containment therefore have one core owner.
