...
const approvedPresentation = new Set([
  ...
  "border:packages/transcript/src/pane-workflow.ts:const branch = isLast ? '└─' : '├─'",
  "border:packages/transcript/src/pane-workflow.ts:rows.push(this.colors.border('─'.repeat(Math.max(0, width))))",
  ...
  "padding:packages/transcript/src/pane-workflow.ts:rows.push(this.colors.border('─'.repeat(Math.max(0, width))))",
