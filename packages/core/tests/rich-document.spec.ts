...
describe('renderMermaidRows', () => {
  ...
  it('returns the source fallback signal for invalid, wide, oversized, and too-narrow diagrams', () => {
    expect(renderMermaidRows('not a diagram', 80)).toBeUndefined()
    expect(renderMermaidRows('graph LR; A[中文] --> B', 80)).toBeUndefined()
    expect(renderMermaidRows('graph LR; A[🙂] --> B', 80)).toBeUndefined()
