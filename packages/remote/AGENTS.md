...
Remote session snapshots start at revision zero and advance once per admitted mux event; buffered replay carries the event snapshot's revision into the rebuilt baseline. Their required `sessionEpoch` is the current per-session attachment epoch, so a detached/re-attached identity cannot be mistaken for the prior generation even when its revision restarts. This keeps the public readonly session fence monotonic within an attachment generation without exposing the remote transport watermark as a renderer or domain object.
