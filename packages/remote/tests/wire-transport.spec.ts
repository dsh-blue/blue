...
describe('DshRemoteTransport', () => {
  it('negotiates health, snapshots, maps events, and tears down streams', async () => {
    ...
    await expect(transport.snapshot('s1', signal)).resolves.toMatchObject({ watermark: 10, value: { cwd: '/fresh', status: 'running' } })
    off(); second(); transport.detach('s1')
    f.emit({ type: 'session/event', sessionId: 's1', event: { type: 'turn/end', seq: 11 } })
    await expect(transport.snapshot('s1', signal)).resolves.toMatchObject({ watermark: 10, value: { status: 'running' } })
