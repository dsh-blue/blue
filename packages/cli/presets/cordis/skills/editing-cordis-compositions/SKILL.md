...
In Blue the host composition is `dsh-base` plus the Blue bundle's `cordis.patch.yml` (three segments: the thin-host disables, the baseline UI rows, the enhancement and assembly rows). A row that changes what the terminal *shows* does not belong in a preset at all — it belongs in the Blue plugin tree or that patch. The `/preset` command switches a blank session's composition honestly; it cannot change the UI that renders the session.
