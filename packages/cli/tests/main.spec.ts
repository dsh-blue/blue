...
const PIN = '0.2.0-alpha.1'
const AHEAD = '0.2.0-alpha.199'
...
function fixtureLauncher(): { calls: { once: Call[], inherit: Call[] }, root: string, hostBin: string } {
  ...
  const host = join(home, 'cache', 'blue-cli-runtime', `${PIN}-0.1.2-alpha.3`, 'node_modules', '@deepseek-ai', 'dsh')
  const hostBin = join(host, 'lib', 'bin.js')
  mkdirSync(root, { recursive: true })
  mkdirSync(host, { recursive: true })
  writeFileSync(join(host, 'package.json'), JSON.stringify({ version: '0.1.2-alpha.3', bin: { dsh: 'lib/bin.js' } }))
...
describe('main', () => {
  it('answers -V with the shell, Blue pin, and harness line in one line', async () => {
    fixtureLauncher()
    await main(['-V'])
    expect(captures.out).toEqual([`blue ${PIN} (Blue @dsh-blue/blue@${PIN} · harness @deepseek-ai/dsh@0.1.2-alpha.3)\n`])
