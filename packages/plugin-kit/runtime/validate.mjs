...
const repositoryRoot = existsSync(resolve(repositoryCandidate, 'packages/api/src/protocol-v1.ts'))
  ? repositoryCandidate
  : undefined
const argument = process.argv.slice(2).find(value => value !== '--') ?? '.'
const root = resolve(argument)
const reproduceTarget = repositoryRoot === undefined ? root : relative(repositoryRoot, root)
const reproduce = repositoryRoot === undefined
  ? `blue-plugin validate ${root}`
  : `node script/blue-plugin-validate.mjs ${reproduceTarget === '' ? '.' : reproduceTarget.startsWith(`..${sep}`) ? root : reproduceTarget}`
const packageFile = resolve(root, 'package.json')
const protocol = repositoryRoot === undefined
  ? await import('@dsh-blue/blue-api/protocol/v1')
  : await import(pathToFileURL(resolve(repositoryRoot, 'packages/api/src/protocol-v1.ts')).href)
const legacyProtocol = repositoryRoot === undefined
  ? await import('@dsh-blue/blue-api')
  : await import(pathToFileURL(resolve(repositoryRoot, 'packages/api/src/manifest.ts')).href)
