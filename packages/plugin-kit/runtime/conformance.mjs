...
const repositoryCandidate = resolve(import.meta.dirname, '../../..')
...
const target = repositoryRoot === undefined ? packageDir : relative(repositoryRoot, packageDir)
const reproduce = repositoryRoot === undefined
  ? `blue-plugin conformance ${packageDir}${harnessLine === undefined ? '' : ` --harness-line ${harnessLine}`}`
  : `node script/blue-plugin-fixture.mjs ${target === '' ? '.' : target.startsWith(`..${sep}`) ? packageDir : target} --install${harnessLine === undefined ? '' : ` --harness-line ${harnessLine}`}`
...
try {
  ...
  if (externalPackage) {
    ...
    const entrySpecifier = distribution.entry === '.' ? manifest.name : `${manifest.name}${distribution.entry.slice(1)}`
    await scenario('plugin.public-entry-packed-load', async () => {
      ...
      ensure(entrySpecifier === (parsed.value.entry === '.' ? manifest.name : `${manifest.name}${parsed.value.entry.slice(1)}`), 'FIXTURE_PLUGIN_ENTRY', 'packed entry resolution drifted')
    ...
    await scenario('plugin.host-admission-width-unload-and-fallback', async () => {
      const probePath = join(fixtureRoot, '.blue-plugin-contract-probe.mjs')
      const probeSentinel = '__BLUE_PLUGIN_CONTRACT_PROBE__'
      writeFileSync(probePath, `
import { Context } from '@deepseek-ai/cordis'
import * as api from '@dsh-blue/blue-api'
import * as core from '@dsh-blue/blue-core'
import { DARK_COLORS } from '@dsh-blue/blue-core/theme-dark'
import * as plugin from ${JSON.stringify(entrySpecifier)}
import { readFileSync } from 'node:fs'

const manifestSource = JSON.parse(readFileSync(${JSON.stringify(join(fixtureRoot, 'node_modules', ...manifest.name.split('/'), 'blue.plugin.json'))}, 'utf8'))

const requested = [...manifestSource.capabilities.required, ...manifestSource.capabilities.optional].map(value => value.name)
...
if (requested.includes('session.projections.read')) {
  ctx.bluePluginControl.attachSessionProjections(ctx, {
    currentMany: keys => ({ asOfSeq: 1, values: Object.fromEntries(keys.map(key => [key, null])) }),
    subscribe: () => ({ disposed: false, dispose() { this.disposed = true } }),
  })
}
...
for (const width of [20, 40, 80, 120]) {
  for (const node of nodes) {
    ...
    for (const row of compiled.value.component.render(width)) {
      if (core.visibleWidth(row) > width) throw new Error(\`plugin row exceeds width \${width}\`)
    }
  }
}
await fiber.dispose()
...
await fallback.plugin(api)
const fallbackFiber = await fallback.plugin(plugin)
await fallbackFiber.dispose()
await fallback.fiber.dispose()
process.stdout.write(${JSON.stringify(probeSentinel)} + JSON.stringify({ capabilities: requested, nodes: nodes.length, widths: [20, 40, 80, 120], unloaded: true, fallback: 'capability-absent' }) + '\\n')
`)
      const probe = spawnSync(process.execPath, [probePath], {
        cwd: fixtureRoot,
        encoding: 'utf8',
      ...
      ensure(probe.error === undefined && probe.signal === null && probe.status === 0 && probe.stdout.startsWith(probeSentinel), 'FIXTURE_PLUGIN_CONTRACT_PROBE_FAILED', `plugin contract probe did not complete: ${probe.error?.message ?? probe.signal ?? `exit ${String(probe.status)}: ${probe.stderr.trim()}`}`)
      ensure(probe.stderr === '' && probe.stdout.trim().split('\n').length === 1, 'FIXTURE_PLUGIN_CONTRACT_STDIO', 'plugin contract probe emitted unexpected output')
      const observation = JSON.parse(probe.stdout.slice(probeSentinel.length))
      report.observations.push({ scenario: 'plugin.host-admission-width-unload-and-fallback', ...observation })
    })
  }
