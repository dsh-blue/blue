...
- **无独立 bin**：profile 经由通用 `dsh` 启动器运行，因此 `installFailLoud` 的 release 是 dispose 整棵树，而不是直接调用终端 release；独立的 Blue bin 会自己把 `createTerminalRelease()` 交给 `installFailLoud`。组装后的 profile 由 bundle 包的全树 e2e 端到端覆盖（`@deepseek-ai/dsh-blue`，`tests/e2e.spec.ts`）。
