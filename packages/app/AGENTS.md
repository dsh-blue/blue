...
The `/agents` browser and the attach view consume four child-addressed boundaries; the current session never switches for any of them. `blueSessionActions.subagentTree()` lists the active session's complete tree in stable pre-order with the host's own `subagents.listDescendants` as the membership authority; resident children additionally carry token/timing metrics folded from their live projections. `childFollowup(childId, blocks)` delivers a human follow-up to one continuable child (the host cold-resumes a non-resident child; one-shot children reject at the host boundary), and `interruptChild(childId)` interrupts one live continuable descendant under exact-ancestor authority. On the read side, `blueSessionProjections.childCut(childId, keys)` re-checks membership, then answers one consistent cut from the projection registry for a resident child or through the optional session-query observation seam for a cold one (read-only, no Agent resumed), while `subscribeChild(childId, listener)` fans live pushes out to any resident `origin: 'subagent'` session, including grandchildren the direct-child routing skips.
