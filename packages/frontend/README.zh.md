...
Renderer-neutral 的 frontend runtime：readonly 交互模型、registry 与 provider host。模型是纯不可变数据——view、list、notification、editor、tool presentation 与 transcript entry——以结构化 `Action` 载荷代替回调。Canonical document 与 status node 使用公开的 `@dsh-blue/blue-api` node contract；footer 和 bottom-pane placement 由 renderer-owned composition 持有。任何模型都不含 pi-tui、React/DOM、ANSI、终端宽度、focus handle、renderer 键绑定、Promise 或 Harness Agent/Session 对象；由 renderer adapter 把它们编译为具体组件。
