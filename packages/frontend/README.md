...
Renderer-neutral frontend runtime: readonly interaction models, registries, and the provider host. Models are plain immutable data — views, lists, notifications, editor, tool presentation, and transcript entries — carrying structured `Action` payloads instead of callbacks. Canonical documents and status nodes use the public `@dsh-blue/blue-api` node contracts; footer and bottom-pane placement belongs to renderer-owned composition. No model contains pi-tui, React/DOM, ANSI, terminal width, focus handles, renderer key bindings, Promises, or Harness Agent/Session objects; renderer adapters compile them into concrete components.
