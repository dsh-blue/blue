...
describe('TranscriptModelService', () => {
  ...
  it('reprojects cached transcript chrome on a locale refresh', () => {
    ...
    const f = fixture()
    const service = new TranscriptModelService(new Context(), f.screen, {
      renderer: renderer(() => {}, undefined, true, translate),
    })
    service.register(model('localized', [
