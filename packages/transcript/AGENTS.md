...
- `./status-title` (`src/status-title.ts`, `blue-status-title`) — enhancement row: the session title folded from the harness `sessionTitle` service (read through a structural interface — no dsh-session-title dependency) at priority 12, `muted`, band 2 left cluster (beside the context bar). Re-derives on load, `'blue/session-changed'`, and the current session's events (the status-basic discipline); untitled/thin-host/no-session render `''` and the entry vanishes. The title itself is generated upstream — the Blue bundle runs the all-prompts cadence, so it tracks the conversation's latest task.
