...
owner attach、aggregate observe、notification observe、gesture mint、semantic overlay close，以及未收窄的 session/projection/action source 都不是公共插件 API。默认 bundle 通过私有隔离 runtime realm 中 closure-bound 的 `bluePluginControl` 承载这些操作；普通 sibling 只能取得受保护的 `bluePluginHost` facade。
