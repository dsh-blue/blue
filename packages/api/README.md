...
Blue-owned adapters receive capability-local `statusRevision`, `statusProvidersRevision`, and `editorExtensionsRevision` snapshot fences. Each capability advances independently, so unrelated mutations do not rebuild the selected status provider or active editor extensions.
