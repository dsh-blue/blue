...
- **复核附录（2026-08-23，用户质疑触发、上游源码级复查）**：背景陈述中"只加 agent-presets 行 = 叠加、`/preset` 撒谎"对**出厂预设**成立（`dsh-scope` 的 `ScopedLayers.merge()` 恒以全局层打底、scope 链只做按名遮蔽；四个出厂预设 `agent.cordis.yml` 零 restrict——minimal 仅 persona(complete)+持久 shell+编辑器），**但"disable 是唯一路径"不成立**：`dsh-tools` 存在公开 API `tools.restrict({allow/deny})`，文档明定"Restrict global tools for the calling agent scope… restrictions intersect; scoped registrations remain visible"——厚 base + `presets.mount` + agent setup 时按预设工具名 restrict，同样可达模型可见面的真替换，且 allow 名单对上游**未来新增行默认拒绝**（漂移免疫方向）。仍维持 disable 路线的裁决理由：① restrict 裁不掉非工具机器行——plan-mode/压缩/subagent 基建全活（minimal 下 plan 模式可进、压缩照跑的半活体缝隙需逐缝逐版审计，税重于已自动化的漂移守卫）；② 官方生态 0 使用（web-app patch disable×26/restrict×0、出厂预设 restrict×0），选它等于自创非官方模式；③ standard 全集的枚举在 restrict 路线下同样面临漂移。当年三选一的选项空间缺此路（方案 2 的"叠加"表述未提 restrict 通道），记档备查：**若漂移守卫成为实际负担，可回落"厚 base + presets + restrict"而不回退预设系统本身**。
...
### D48. 渲染宽度崩溃族根治：出口兜底 + 真语义测试网 + 真进程 smoke（#14/#15/#18，2026-08-22 用户裁决；原编 D45，与 PR #19 已并的 /sessions D45 撞号，重编——D37→D39 先例）
...
### D49. paste-image 失败分级提示 + 多类型探测：提示缺失而非代装（用户裁决，2026-08-22；原编 D48，与 PR #20 宽度根治撞号，重编——D37→D39/D45→D48 同款先例）

- **背景**：SSH 会话启动的 Blue（进程无 `DISPLAY`/`WAYLAND_DISPLAY`/`XAUTHORITY`）Ctrl-V 报 `no image available from the clipboard`。实证根因三层叠加：`wl-paste` 未装、`xclip` 无 display 报 "Can't open display"、读器把「工具缺失/够不到显示会话/剪贴板没图/超时」全部折叠成同一条 no-image 提示——误导用户以为"数据过去了但丢了"。同类误导第二处：探测写死 `-t image/png`，剪贴板里是 JPEG 时链路全通也报 no image。
- **决策（用户裁决）**：**不代装、不绕过、提示缺了什么**——否决 `systemctl --user show-environment` 环境自动发现等绕过路线；多类型探测一并做。读器契约改判别联合 `ClipboardImageResult`（`image`/`no-image`/`unsupported`/`unreachable`/`missing-tool`/`timeout`/`failed`+detail），六级提示各报所缺：`clipboard image tool missing: install wl-clipboard (wl-paste) or xclip`、`clipboard unreachable: DISPLAY/WAYLAND_DISPLAY is not set in this session`、既有 no-image、`clipboard image type image/bmp is not supported`（带实际类型）、`clipboard read timed out`、`clipboard read failed: <detail>`（detail 仿 clipboard-write 的 `exited with code N: stderr 首行` 习语）。**进行中提示（验收二轮反馈 2026-08-22）**：Ctrl-V 按下即闪 `pasting image...`——探测可达秒级（挂死的 wl-paste 付整段超时），空等读作死键；成功时标记插入触发的编辑器 onChange 自行清掉该行，失败被结果提示原位覆盖，unload 边界下残留至下次编辑（无清除 API，记边界）。
- **多类型探测**：两步——`wl-paste -l` / `xclip -selection clipboard -t TARGETS -o` 列类型，与准入集取交集后按准入序（png>jpeg>webp>gif）逐个 `-t <type>` 读；零字节、软失败、无法识别或字节魔数与所请求 MIME 不符均试下一 representation，避免坏 PNG 挡住同源有效 JPEG，硬失败（missing/timeout/unreachable）才止住当前后端。嗅探只判候选有效性，绝不把声明类型改写成另一类型；完整解码、尺寸/限额与最终交叉校验仍归 store。准入序与扩展名单一来源：attachments 导出 `ADMITTED_IMAGE_TYPES`（store 白名单从它派生）与 `EXT_BY_MEDIA_TYPE`（`pasted-image.jpg` 等文件名派生）。
- **聚合优先级**（rank-map 查表）：`unsupported > no-image > unreachable > failed > timeout > missing-tool`。completed query 最有发言权（X11 会话 wl-paste 必然连不上，xclip 查明没图 → 报 no-image 才对）；timeout 高于 missing-tool——wl-paste 挂死 + xclip 未装时报"缺工具"是撒谎（wl-paste 在，真诊断是挂死）。**环境门**：unreachable/missing-tool 混合时按 env 裁决——`DISPLAY`/`WAYLAND_DISPLAY` 已设 → 报 missing-tool（装上缺席工具即真修好：X11 会话只装 wl-paste 的混合场景）；皆未设 → 报 unreachable（无头场景）。
- **后果/边界**：配置 `backend=auto|wayland|x11` 默认 auto；auto 按显示环境让 Wayland 或 X11 原生后端领先，严格模式不跨协议。最坏首轮墙钟仍为 12s，但 timeout 按 backend + DISPLAY/WAYLAND_DISPLAY/XDG_RUNTIME_DIR 键冷却 60s，主题 reload 不丢、环境变化或到期自动复探；只缓存 timeout，不缓存 no-image/unsupported/普通失败。**超时击杀用 `killSignal: 'SIGKILL'`**——Node 默认 SIGTERM 杀不掉 trap TERM 且卡死的 wl-clipboard，exit 事件不来会让读器永挂；spec 以忽略 TERM 的假 bin 钉死。GNOME/mutter 无 zwlr data-control，后台 wl-paste 可能恒超时，xclip/Xwayland 因而是实际回退；若 Wayland→X 剪贴板桥损坏，X11 侧可能仍持有旧图，程序无法从 selection 协议判断“用户刚复制的那张”。auto 跨后端成功因此在 marker 插入后提示实际 fallback，要求核实；需彻底规避旧图时用严格 backend。xclip 仅其 TARGETS 静默非零特例映射 no-image，wl-paste 同形失败保留为 failed；clipboard-write runner 不合并，website FAQ 按 R6a 冻结不同步。
- **落地（2026-08-22，S36）**：paste-image 重写（契约 + 两步探测 + 分类/聚合 + 候选字节校验与 representation 回退 + backend 配置/环境顺序 + timeout 冷却 + fallback 警示 + 六级/进行中提示）+ attachments 双导出 + spec 全量矩阵（逐文件 100%，含显示键换代、冷却到期和坏 PNG→有效 JPEG）+ clipboard-write EACCES 用例根治 + bundle e2e 经真实 FilesystemAttachmentStore 逐字节回读。**真机验收状态：代码门禁后仍须用 `blue-s36-paste` 完成桌面源 PNG/JPEG 与 auto/strict backend 验收；通过前勿并。**
- **文件管理器验收反馈与追加裁决（2026-08-22）**：Ubuntu Files 复制 `castle.png` 时实测 Wayland/X11 均只声明 `text/uri-list`、`x-special/gnome-copied-files` 和文本类型，内容为 `file:///tmp/logo/castle.png`，不声明 `image/png`；原读器据协议返回 no-image 虽属实，却不符合用户对“复制图片文件即可粘贴”的自然预期。直接 image representation 继续最高优先；其后按标准 `text/uri-list` → GNOME copied-files 读取一个有序批次（`cut` 只读、不移动源文件），只收唯一的本地 `file:` URI。文件以 Node URL/fs API 打开，末级 `O_NOFOLLOW`、`fstat().isFile()`、附件部署的数量/单图/总字节限额做有界读取，媒体类型只取魔数嗅探，显示名只取 basename；远程 scheme/host、目录、末级符号链接、特殊文件、坏 URI 与混合非图片批次整批拒绝。整批经 `AttachmentStore.saveImages` 先验后提交，成功才一次插入有空格分隔的多个 marker；底层中途写失败可能留下不可达对象是上游契约已记边界，编辑器不出现半组 marker。规格追加 URI CRLF/注释/百分号编码/去重、GNOME cut、直接图片优先、坏直接 representation→文件回退、多选及全拒绝矩阵。真进程伪终端以 X11 `text/uri-list` owner 提供同一 `castle.png`：出现 `[image #1]`、退出 0、bracketed paste 成对，附件对象与源文件 SHA-256 同为 `a0efa2a8587486ce9a5a0a94f474104b9db1ea971fb9a41820a8e8c9b99d6f62`。**新增 Ubuntu Files 原生 owner 路径仍须用户用 `blue-s36-paste` 真机复验，通过前勿并。**

### D50. 发版管道与安装形态：CI-only 发包 + dist-tag `rc` + rc.2 `blue` 壳（用户四裁决，2026-08-22）

- **背景（发包前核实）**：npm org `dsh-blue` 尚不存在（`npm owner ls` 404，发包硬前置）；五包 exports 均带 `./src/*` 而 `files` 不含 `src/`——tarball 死出口；`@deepseek-ai/dsh` 公开在册（latest=next=0.1.1-rc.2，壳可精确钉装）、`@earendil-works/pi-tui@0.84.2`=latest、`@deepseek-ai/*@0.1.1-rc.2` 在 next tag 下精确钉版可解析（dist-tag 不影响 range/精确解析）；`dsh plugin add` 实测即 profile 工作区 `pnpm add` 薄封装（支持 `name@tag`，无自有默认 tag 逻辑）；npm 包名 `blue` 已被占（v1.1.1）——壳只能 scoped 包 + bin 名；仓库 `dsh-blue/blue` 公开（provenance 可用）；website 已 `private: true`（`pnpm -r publish` 自然跳过）。
- **决策①（CI-only 发包，从不手动）**：发版权威 = master 提交上的 `vX.Y.Z-*` tag → `release.yml`：tag==manifest 守卫（version.spec 六 manifest 锁步的无 vitest 快读）+ tag 须为 master 祖先 + 全量 gate 复跑（typecheck/lint/diagrams/build/check:lib/test:coverage；smoke 留 ci.yml——tag 指向的提交已在 master 过过）+ 显式 `.npmrc` 写 token + `pnpm -r publish --tag rc --no-git-checks --provenance`（拓扑序自动；--no-git-checks 因 tag checkout 是 detached HEAD）+ `npm view` 五包验证。凭证 = org 级 **granular token**（GitHub secret `NPM_TOKEN`；Read and write、**All packages**——首发时包不存在，按包名选覆盖不到首发布；与 2FA 并存、自动化通道不问 OTP）。**Trusted Publishers 起步不用**：配置入口要求包已存在（npm/cli#8544，无 PyPI 式 pending publisher）且 pnpm tokenless OIDC 未成熟（pnpm#9812）；rc.1 存在后可迁（工作流只改认证行）。
- **决策②（dist-tag `rc`，不占 latest）**：五包只打 `rc`，latest 留给稳定线。由此**一切手动安装命令一律带 tag 后缀**：`dsh plugin --profile blue add @dsh-blue/blue@rc`——pnpm 默认解析 latest，无后缀即 "No matching version"（R4 原定命令改写依据）。
- **决策③（`./src/*` 发前清）**：五包 exports 删该行（rc.1 前是最便宜窗口；blue-api-design Phase A 的"关闭意外暴露的源码入口"清单该项提前完成）。`check:lib` 门禁本就跳过通配出口、仓库零 `/src/` 子路径消费者，纯声明删除。
- **决策④（rc.2 壳包，A 方案 + 托管校准）**：新包 `@dsh-blue/blue-cli`（bin 名 `blue`，仓库 `packages/cli`，version.spec 扩第七份 manifest）。**A 方案**：dependencies 精确钉 `@deepseek-ai/dsh`——嵌套 dsh 即运行时宿主，用户全局有无 dsh 皆可、版本恒与测试线一致；**托管校准**：壳精确钉 bundle 版本，启动读 profile 清单（`$DSH_HOME/profiles/blue`），`@dsh-blue/blue` 版本缺失或不等才调嵌套 dsh `plugin add`（一致零开销直通）；升级 = 重装壳一条命令（`blue upgrade` 子命令因此不需要）。profile 名固定 `blue`（与 `dsh --profile blue` 同工作区同会话；验收惯例 `blue-<tag>` 不同名天然不撞，dev link 不受校准波及）。**参数归属**：`-V/--version` 壳自答（壳/Blue/harness 线一行三段）；`--patch`/`--dump-config`/`plugin` 子命令翻译透传（`--profile` 壳吞掉恒为 blue，未来 Blue 加参数不与宿主撞车）；其余参数原样交 Blue——解析本就在 startup.ts（cmdlineArgs 注入 + commander），壳只换品牌前缀（`program.name` 与 help 示例随壳改 `blue`）。**壳必须独立全局包**：bundle 装在 profile 工作区，其 `node_modules/.bin` 不进 PATH，bin 塞 bundle 里无效。bootstrap 失败（网络/registry）一行报错 + 指路手动命令，非零退出（失败形态经 D56 扩展：verdict + 可选输出尾 + 按类 manual）。
- **理由**：CI-only 使凭证不落本机、每次发版同一管线自带审计，发版动作收敛为打 tag（rc.2 壳与 harness 线跟发复用同一管道，边际成本趋零）；@rc 保住 latest 稳定语义（rc 期间不换安装心智——rc.1 用户走 dsh 命令，rc.2 起 quickstart 双写）；壳把"参数解析已在 Blue 侧"的既有缝变现为独立命令，dsh 组合层（--patch/user layer/config 组合）零损失，不重写宿主。
- **后果/边界**：发版流程 = bump 六（rc.2 起七）manifest + `BLUE_VERSION` + website 口径 → PR → master → `git tag vX.Y.Z-N && git push origin vX.Y.Z-N`；release.yml 部分失败恢复——pnpm 不跳过已存在版本，勿整组重发，按 verify 步所示只补缺尾；pnpm 11 `minimumReleaseAge` 环境会拒刚发布的包（R1 三坑同源），R4/用户"装不上"先查冷却期；granular token 到期 npm 邮件提醒，轮换 = 重生成 + `gh secret set`；rc.1 后迁 Trusted Publishers 可选。**跟发触发规则已定档 D53**（跟发=自主节奏，drift-monitor 提示不构成义务；原措辞"harness 出新 rc → Blue 何时跟发 rc.N——drift-monitor 自动 bump PR 会积压，rc.2 后需定档"）。
- **落地（2026-08-22，worktree 待人工验收）**：release.yml + 五包 `./src/*` 清除 + roadmap R3/R4 行更新与 rc.2 节（S37）+ 本条记档（本 PR）；npm org/token 人工步骤用户侧完成；另随批修复 master roadmap 一行孤儿冲突标记（224eb14 S36 rebase 遗留，内容无伤）。
- **S37 落地（2026-08-23，worktree 待人工验收）**：`packages/cli` 按决策④成型——A 方案嵌套宿主（`@deepseek-ai/dsh@0.1.1-rc.2` 唯一依赖）、托管校准（版本比对读 profile `node_modules` 实况、`link:`/`file:` 泳道跳过、安装走嵌套 dsh `plugin add` 精确钉版 + 装后复验）、参数翻译（`-V` 三段自答 / `plugin` 子命令 `--profile` 后置 / 其余顶层直通 + 用户 `--profile` 吞没）、`BLUE_LAUNCHER=blue` 子进程环境换品牌（startup 帮助与退出遗言两消费点）。**两处执行层裁定**：① updater 核心不 import——给插件包开 exports 子路径即 S30 事故族，壳自含 ~30 行 profile 读取（D52「复用核心」按此落地）；② dsh 宿主入 .pnpm 使 loader internal importer（node-addon-require-builtin）激活，/mcp e2e 裸名行改从临时 profile baseUrl 解析——e2e-boot 链 dsh-mcp-client 进临时目录（与真 profile 语义等价），两 importer 分支皆绿。version.spec 扩八 manifest + 壳裸 dsh 钉版断言；release.yml verify/hygiene 双循环加 blue-cli；quickstart 双语双写 + FAQ 升级双路。
- **管线实测记要（2026-08-22，B/C 测试，PR #30 评论存档）**：C（tag 守卫）——分支 tag 触发后死在守卫二（`tag commit is not on master`），推演顺带抓出守卫在 fetch-depth:1 浅克隆下假失败的 bug（已修全历史 checkout）；B（canary）——`test/npm-smoke` 分支经 NPM_TOKEN 发布 `@dsh-blue/smoke`，首发 EOTP 证实 secret 首版 token 系 Classic Publish 型（换 granular 后绿）。**canary 实测 npm 行为：包的首次发布必自动写 `latest`，即使显式 `--tag canary`**——若不处理，五包首发会自动占 latest=rc，违反决策②；release.yml 补 dist-tag hygiene 步（latest 指向刚发 rc 时移除；未来刻意 `--tag latest` 的稳定版发布不满足条件、不受影响）。

### D51. rc.1 tarball 自毁修复：files 整目录 + 桥依赖自带 + 发版前本地 pack 装机验证（用户实测踩出，2026-08-22）

- **背景**：rc.1 五包发布成功、干净环境装机即死——PTY boot 冒烟 16× `ERR_MODULE_NOT_FOUND` + `Cannot find package '@deepseek-ai/dsh-session-title-all-prompts-llm'`。两根因：① tsdown 会产出带哈希的共享 chunk（`present-*.js`/`theme-palette-*.js` 等），不占任何 exports 条目，`files[]` 的逐文件枚举必然漏装——tarball 里入口 import 第一个 chunk 就 404（S30 只守住了 exports↔files↔tsdown 三方对齐，chunk 是盲区；dev profile 走 link 安装、结构上永远看不见 tarball 内容）；② session-title 桥引用的 `all-prompts-llm` 在上游 rc.2 未收进 dsh-base（R1"桥未修保留"的另一面），monorepo 时代靠 workspace 链接侥幸存在，出树即失踪——bundle 自己不带依赖就没人带。另修一处装机命令文档 bug：`--profile` 是 `plugin` 子命令的必选项，`dsh --profile blue plugin add …` 必报 `required option not specified`，正确形式 `dsh plugin --profile blue add <spec>`（顶层 `--profile` 是 boot 选项）。R3 的 B/C 测试均未触及真实 tarball 装机（canary 是单文件包）。
- **决策①（files 整目录）**：五包 `files` 统一 `["lib/**/*"]`（bundle 另加 `cordis.patch.yml`）——哈希文件名构建时才定，枚举式白名单结构性漏文件；`check:lib` 补反向遍历（lib/ 下每个构建产物必须被 files 覆盖，负向测试在档：收窄一包即吐 137 问题、含今日全部肇事 chunk）；`tsconfig.tsbuildinfo` 移出 lib/（各包 tsconfig `tsBuildInfoFile` → `node_modules/.cache/blue-tsb/`），lib/ 恢复"纯可发布产物"语义。
- **决策②（桥依赖自带）**：`@deepseek-ai/dsh-session-title-all-prompts-llm` 从 bundle devDependencies 升 dependencies（精确钉 `0.1.1-rc.2`，与 agent-presets/mcp-client 同款——运行时经 profile 工作区解析）。上游把包收进 dsh-base 后可降回。
- **决策③（发版前本地 pack 装机验证）**：发版预检新增一环——`pnpm pack` 五包 → scratch `DSH_HOME` profile 装入（bundle tarball 直装 + `pnpm-workspace.yaml` overrides 把四个库钉到本地 tarball）→ PTY boot 冒烟（零 `ERR_MODULE_NOT_FOUND`/`Cannot find` + banner 四行）→ 才准打 tag。rc.2 按此验证后发车；R4 的装机验证以此为固定门。
- **决策④（latest 落点）**：registry 禁删 latest（`npm dist-tag rm` 一律 403，实测）；包内只有 rc 线时 latest 被 registry 锁死在 rc 上（release.yml hygiene 步已改为"有 stable 指回 stable、无则告警放行"）。rc.2 发布后手动把五包 latest 指到 `0.1.0-rc.2`——裸安装落到能跑的 rc.2 优于落到自毁的 rc.1；0.1.0 stable 发出后按 D50② 自然回归稳定线。
...
- **决策①（载体与范围切分）**：全部新代码收在 `blue-interaction` 包内（`src/updater/` 七模块 + `update-command.ts` + `update-notice.ts`），零新包、零新依赖边、零 cordis.patch 变更——启动检查的通知 UI 与命令的对话框都是 interaction 原生。D50④"壳不需要 upgrade 子命令"只约束未来的 `@dsh-blue/blue-cli` 壳自身；`/update` 服务的是 `dsh --profile blue` 直装的存量用户，二者互补，S37 落地后壳的托管校准直接复用 `updater/` 核心。
- **决策②（启动检查，元数据 ≠ 远程内容）**：新插件 `blue-update-check` 随 baseline 挂载（index.ts apply），整树 loader settle 后 fire-and-forget（绝不与 boot 抢资源，unload flag 守护续体）；一次 `npm view @dsh-blue/blue --json`（15s 超时、2 次重试 1.5s/4s 退避；走用户 npmrc 镜像/代理）→ npm 缺失兜底直连 registry API（8s）；24h 缓存于 `$DSH_HOME/storages/blue-update/state.json`，一切失败静默。**边界定性**：这是 registry 元数据读取，不是远程内容——kimi 远程 banner 的否决仍然成立（一次 GET、本地只存时间戳/版本、零上报），settings `blue` 命名空间 `updateCheck: false` 即完全离线。通知 = 滚动区两行 `UpdateNoticeComponent`（banner 纹丝不动——S35 三轮定稿）+ 编辑器 hint 闪现，内容含 `/update` 与精确安装命令双指引。`updateChannel`（默认 `rc`）留通道位，stable 线存在后 `latest` 零新代码可用。
- **决策③（六道预检，全部纯函数化）**：① link/`file:` 污染 → 阻塞 + 全组精确版本修复配方（三泳道）；② 六包组一致性（Frankenstein 守卫）；③ 目标须在 packument versions 里；④ D51 下限 `0.1.0-rc.2`（`VERSION_FLOOR`，回滚目标同受辖）；⑤ 宿主线——读目标版本 manifest 里 `dsh-agent-presets` 精确钉版，宿主过旧 → 阻塞给 `npm i -g @deepseek-ai/dsh@<line>` 精确命令，跨 minor 更新 → 警告不阻塞（R1 裁决），不可探 → 警告交冒烟裁决；⑥ 冷却期预判——目标发布龄 < `minimumReleaseAge`（profile cwd 探测，读不到按 pnpm 11 默认 1440min）→ 阻塞给 ETA 绝对时间，**绝不自动绕过**（供应链守卫是用户的政策）。
- **决策④（swap 执行器：快照 → 单事务 → 双层冒烟 → 自动回滚）**：快照 profile 三文件（package.json/pnpm-lock.yaml/cordis.patch.yml）至 `<profile>/.blue-update-backup/`（随 profile 自包含，保留一份覆盖式）；`dsh plugin --profile <name> add @dsh-blue/blue@<精确>` 单事务安装；装后重读六包版本组校验；冒烟 A（必跑）= 新 Node 子进程 `createRequire`+`import()` 扫 bundle `cordis.patch.yml` 全部 `name:` 条目 + 三个钉版运行时依赖——精确抓 D51 级缺 chunk/缺依赖，无终端无凭证 ~2s；冒烟 B（默认开）= pipe-stdio 真 boot（`smoke-happy.mjs` 形态：默认模型串 marker → `/quit` → 双 Ctrl-C 阶梯 → exit 0，无 node-pty；无 marker 退化 15s 存活判定）。任一失败 → 恢复快照 + 六包旧精确版本全组重装 + 复冒烟（回滚不信 `^` 范围）；起点低于下限（rc.1 用户）不回滚、留下限版本手工配方。安装失败按日志尾分类翻译（冷却期/网络/ETARGET/权限/通用）。进度经 `UpdatePanel` 步骤梯呈现，install/smoke 进行中忽略 Esc（中途关面板是唯一危险动作）。
- **决策⑤（运行中升级语义）**：live 进程持旧码继续跑（五包已验证全静态导入、零运行时动态 `import()`），成功后醒目"重启生效"提示；上游侧懒加载残余风险记为已接受。bare `/update` 即只读检查（已最新直接回答），`/update <精确版本>` 显式钉目标；`/version` 不动（version.spec 钉住）。
- **理由**：R1（冷却期三连：元数据解析 + 精确钉版 + 预检 + 报错翻译）与 D51（下限 + boot 冒烟作为装机时门禁）的运行时化——每条发布期教训变成一道用户侧门禁；目标解析读 packument 而非包管理器解析结果，静默回退结构性不可见。
- **后果**：`/update` 入 blue 命令组（/help 列表 39 行，两处钉数断言随更）；settings `blue` 命名空间首个消费者（`installSettingsSection` + MemorySettings 真服务测试）；随批修复 `check:lib` 通配匹配器嵌套目录 bug（`lib` + `**` + `*` 型 files 条目此前只匹配一层子目录——占位符两段替换修复；嵌套 types 目录是首个触发者）；banner.spec 的 cwd 断言改深度无关（worktree 深路径暴露 compose 截断计 ANSI 的既有怪癖，另行观察未动 S35 面）。**跟发触发规则已定档 D53**（原措辞"本机制不解决 Blue 何时跟发上游 rc"）。
- **落地（2026-08-23，worktree 待人工验收）**：`updater/`（version/io/registry/profile/preflight/swap/check）+ `update-command.ts` + `update-notice.ts` + index/commands-plugin 挂载 + specs 8 份（per-file 100%）+ 本条记档 + roadmap S38 行 + FAQ/README/website 双语（本 PR）。
- **dogfood 三发现（2026-08-23，全部随批修复并有回归）**：① 命令错误结果按**单行截断**渲染，多行修复配方根本不可见——阻断判定与失败结局改走 UpdatePanel（消息行 `wrapText` 换行不截断），结果文本只留单行短摘要；② 硬编码六包集合 vs rc.2 实发五包（blue-api 未发布）——verify 误判健康树、回滚向 registry 索要从未发布的包；包集合改为按目标版本动态推导（`releaseFacts`），组一致性门改判"已装集合的版本混杂"（缺失成员由安装自然补齐，成员齐全性归装后 verify 对着目标集合管）；③ `npm view --json` 的 `versions` 是**版本字符串数组**、任意版本的依赖清单不在其中——normalizePackument 兼容两种形状，缺 deps 时经 `npm view pkg@version dependencies --json` 定向补查。演练存档：rc.2→rc.2 全链路 no-op 16s 过（真装幂等 + 真导入扫描 + 真 boot 冒烟）；rc.2→rc.1 真地雷 5s 内被装后校验抓住并整组回滚 rc.2（docs/history/blue-update-dogfood-s38.md）。

- **发版与真机矩阵（2026-08-23 续）**：rc.3（blue-api 首发六包，PR #39）与 rc.4（PR #40，修 target-exises 误判——npm-view packument 的 versions 是**裸键**，按值判定把一切已发布版本判为未发布，rc.3 上任何真实升级都会被挡；改 `Object.hasOwn` 判键存在）先后经 release.yml 发出。真机矩阵（scratch profile + PTY 驱动，全过）：①rc.3 会话启动检查真通知"rc.4 available"；②bare `/update` up-to-date 只读路径；③**冷却期预判门真实拦截**（rc.2/rc.3 均在 24h 窗内 → 面板给绝对 ETA 并拒动；`minimumReleaseAge: 0` 的用户关窗姿势可解，FAQ 已载）；④降级尝试（rc.4→rc.2）单事务后兄弟包滞留 rc.4 → **装后校验抓住混树 → 真实自动整组回滚 rc.4 → 复冒烟通过**——安全链全程真机验证；⑤成功路径 TUI 全程 12s（confirm → 真装幂等 → 真冒烟 → "restart dsh to apply"）。**新知两条**：(a) 降级方向单 spec 事务天然留旧兄弟（范围不回拉）——降级需整组 spec（后续可让 swap 在 from>to 时自动用全组安装）；(b) 回滚的整组 spec 应取 **from 版本的集合**（本次用的目标集合，rc.4→rc.2 场景下恰好无害）——两处记为 D52 后续微调。npm 新包 404 读延迟约 3 分钟（blue-api 首发实测），重试即过。latest 手动迁移（六包 → rc.4）因本机无 token 留用户侧执行（D51④ 同款）。

- **加固批次（2026-08-24，22 项发现随批落地，worktree 待人工验收）**：①新知 (a)(b) 销账——降级目标时 swap 自动改用目标发行版**整组**精确 spec 单事务（bundle-only 会留新兄弟触发回滚），预检新增第七门（降级警告，非阻塞）；回滚集合改取 **from 版本自身的发行集合**（`releaseFacts(packument, fromVersion)`，registry 不答出成员时回退到实况已装集合——profile manifest 只声明 bundle，快照 package.json 同样答不出）。②冒烟 A 范围对齐决策④原文：扫描对象 = patch 全部 `name:` 行（含 dsh 宿主行）+ bundle manifest 声明的 `@deepseek-ai/*` 运行时依赖（`declaredRuntimeDeps`），豁免收窄为"未声明的 @deepseek-ai 目标"（宿主 boot 时提供的 peer，如 cordis），`import()` 目标经 `pathToFileURL`（Windows 盘符裸路径被 ESM loader 拒）。③swap 全程括号化 `pending.json` 崩溃标记（快照后写、成功/完整回滚后删），启动检查读到标记即滚动区挂"上次 /update 被中断、profile 可能混树、备份路径即修复"警告行。④启动检查三件：`lastOffer` 缓存命中重启重挂通知（原先只有当场才挂）、失败读**不烧** 24h 窗口（保留旧 lastCheckAt，下次启动即重试）、link 泳道 profile 抑通知（通知邀请的 `dsh plugin add` 正是半覆盖凶手）。⑤registry 读新增 E404 独立分类（镜像错配 ≠ 断网）与 `onRetry` 钩子，命令侧 hint 行闪现"checking the registry…/retrying (n/3)"。⑥杂项：update.log 256KB 截尾封顶（留尾 64KB）；快照改 staging+rename 原子替换（清陈旧 .tmp）；`/update` 进程内 in-flight 守卫（"an update is already in progress"）；swap 抛异常（ENOSPC 类）兜底 settle 面板（未 settle 的面板按设计拒关，会把编辑器槽位永久劫持）；`INSTALL_TIMEOUT_MS` 导出（20min 安装超时常量单一来源）。e2e-boot 的启动检查走 seam 离线化（models.dev 同款先例），不引 settings 服务（保住 '/provider add 无宿主 settings' 用例）。

### D53. 跟发节奏定档与锁步钉版的退役条件（2026-08-23，S37 壳落地后的架构复核产物）

- **背景**：S37 壳（A 方案嵌套钉宿主）就位后，跟发动力学反转——上游破坏性更新对 Blue 用户（存量与**新增**，精确 spec 不随 registry 漂移）均无影响，全局 dsh 升降级与 `blue` 无关。D50/D52 两处"跟发触发规则仍开放"悬而未决，且钉版纪律（锁步+漂移守卫+跟发）作为 pre-release 时代的桥需要明示彼岸。对照系复盘：官方 web-app 靠**树内发货**（dsh 依赖 dsh-web-app/dsh-base，installation-first 解析契约）免费获得一致性；dsh-TUI 树外浅消费靠浮动带+迭代速度吸收不确定性；Blue 树外深消费自建一致性——三家策略与消费深度严格对应。
- **决策①（跟发节奏）**：**跟发 = 自主节奏，不构成义务**。触发仅三种：需要上游新能力、上游安全修复、harness 稳定线临近需要演练放宽。drift-monitor 的 BUMP_READY 提示不具行动强制性（积压为正常态，必要时调低频率）；R1"跨 minor 只报不动"延伸为"跨 rc 默认不动"。
...
### D55. paste-image 跨平台：win32/darwin 原生探针（PowerShell/osascript staging 协议）+ Windows Alt-V 双绑定 + 复制文件批次对等（用户裁决，2026-08-24）

- **背景**：paste-image 此前只认 Linux 的 `wl-paste`/`xclip` 并以 `DISPLAY`/`WAYLAND_DISPLAY` 门控——Windows/macOS 上按粘贴键要么动作不触发（Windows Terminal/conhost 截留 Ctrl-V 做自己的文本粘贴，0x16 字节到不了应用），要么闪出误导性的"install wl-clipboard"提示。用户裁决：Windows/macOS 与 Linux **完全对等**（图片字节 + 复制文件整批）；Windows 键位 alt+v+ctrl+v 双注册；WSL 桥接出范围。
- **决策①（win32 探针，单 spawn staging 协议）**：`powershell.exe -NoProfile -NonInteractive -STA -Command <脚本>` 一次调用。读剪贴板用 **WinForms `ContainsImage`/`GetImage`**——否决 `Get-Clipboard -Format Image`（PrintScreen/Win+Shift+S 的原始 CF_DIB 位图它处理不了）；`Add-Type -AssemblyName` 装预编译程序集、无 csc 运行时编译（不触发 EDR）。交接一律走 **staging 目录 + env `BLUE_PASTE_STAGE`**：二进制与路径绝不经过 PowerShell stdout（控制台码页 mojibake 与 base64 折行两条已知坑）；脚本写 `kind` 标记（`image`/`files`）与 `clipboard.png`，或把 FileDropList 复制进 `files/` 子目录（原顺序进 `order` 清单、同名冲突改名、目录跳过、全目录选择 exit 2），退出码 0/2/1。staged 副本是脚本自建普通文件——win32 读路径没有符号链接面（`O_NOFOLLOW` 本就 POSIX-only）。超时 10s（PS 冷启动 ~0.5-1s+）。
- **决策②（darwin 探针，列类目再读）**：osascript 先 `clipboard info` 列类目（**禁用** Claude Code 的 `the clipboard as «class PNGf»` 检查式用法——整图以十六进制转储 stdout，默认 maxBuffer 直接 ENOBUFS，anthropics/claude-code#29776）；再按 PNGf > JPEG picture > GIF picture 直读（AppleScript `open for access` 写 staging、`set eof` 截断）。**TIFF→`sips -s format png` 转换必做**：Chromium/Electron 系（Chrome/Figma/VS Code webview）只放 `public.png` UTI 不放旧式 `«class PNGf»`（#30936），但必放 TIFF；两种拼写 `«class TIFF»`/`TIFF picture` 依次尝试。Finder 复制的文件走 `«class furl»` 列表（POSIX path 逐行 `return`；osascript stdout 为 UTF-8——全探针唯一走 stdout 的路径清单）。残余图像类目（8BPS/jp2/BMP/PICT/ICO/icns）以 unsupported 点名。超时 5s。
- **决策③（接线）**：`ClipboardBackend` 扩为 wayland/x11/win32/darwin（Linux 表改键 `LinuxBackend`，表内容零改动）；`backend` 配置**仅 Linux 生效**，win32/darwin 平台自动选原生探针（schema 不动，零迁移风险）；D49 的超时/冷却/聚合机制原样沿用——单后端平台的聚合退化为单裁决，unreachable+missing 混合结构性不可达；missing-tool 文案按平台点名（powershell.exe / osascript）；win32/darwin 恒 index 0 → fallback 永不置位，标签三元改穷尽。复制文件三平台共用 `readCopiedPaths`（去重 + 数量/聚合上限 + 不跟随符号链接 + 魔数定型，URI 解析只留 Linux）。
- **决策④（键位）**：win32 双注册 `alt+v` + `ctrl+v`——WT/conhost 吃 Ctrl-V 的场景用 alt+v（Gemini CLI 惯例），VS Code 终端等透传场景保住 ctrl+v 肌肉记忆；其余平台维持 ctrl+v（macOS 终端粘贴是 Cmd-V，Ctrl-V 原样到达）。
- **后果**：`probeWindows`/`probeDarwin` 落 `paste-image-native.ts`，共享原语（`runTool` 选项化 / `classifyNativeFailure` / `readCopiedPaths` / `withStagingDir`）落 `clipboard-probe.ts`——均为内部兄弟模块（`src/updater/` 先例，exports map 不动、经 paste-image 入口打包）。进程中途被杀可能残留有界 `blue-paste-*` staging 目录（已记边界）；`clipboard info` 类目拼写随 macOS 版本漂移时按 no-image 兜底并文档记档；WSL（linux 平台）行为不变，FAQ 记为已知限制。已知留验项：CF_DIB 透明区压黑（个别截图工具）、TIFF 多帧塌缩首帧、alt+v 在 AltGr/kitty 协议终端的可达性——均入真机验收清单。真机验收（Windows 10/11 + macOS，`blue-s39-paste` profile）为合并门禁，**通过前勿并**。
- **落地（2026-08-24，worktree 待人工验收）**：clipboard-probe/paste-image-native 两个新模块 + 平台缝（`backendsFor`/`pasteKeysForPlatform`/`setClipboardPlatform`）+ native spec（PATH 注入 node 假工具——shebang 钉 `process.execPath`，`env node` 在被替换的 PATH 下解析不到；脚本字面量按字节 pin）+ 平台缝 spec（双键/文案/冷却端到端）+ 本条记档 + roadmap S39 行 + README/AGENTS/patch 注释/website 双语（本 PR）。

### D56. 壳 bootstrap 过渡修复：pnpm 预检 + win32 9009 判类 + 失败上下文保留 + manual 分流 + 1200s 预算（用户裁决，2026-08-24）

- **背景**：rc.5 后 Windows 用户真机实测暴露首装四问题，全链已复现定位。①核心 bug：dsh 的 `plugin add` 在 win32 用 `spawnSync("pnpm", args, { shell: true })`——cmd 找不到命令返回**退出码 9009** 而非 ENOENT error，dsh 的 "pnpm not found on PATH" 分支永不触发、改报误导性的 "pnpm failed in profile directory"；壳的 `/pnpm not found/i` 翻译随之失效（实测：`blue` → "bootstrap failed — dsh: pnpm failed in profile directory C:\Users\…"，实测验证 shell:true 下命令缺失 error=undefined/status=127·9009）。②失败上下文丢失：`lastLine` 单行契约把 pnpm 真实报错（ERR_PNPM_*）全吞，排障只能翻源码。③manual 指引不可执行：失败提示 `dsh plugin …`，npm 不链接传递依赖 bin，新用户无全局 dsh（实测 'dsh' 不是内部或外部命令）。④300s 预算偏紧：慢网络用户 npm 装 455 包实测 18 分钟，profile 内 pnpm 装配同受网络支配。附带实测：pnpm 全局装 blue-cli 因严格隔离布局在 boot 阶段 `ERR_MODULE_NOT_FOUND`（嵌套宿主依赖解析不到）——npm-only 泳道规则实测成立，文档需警示。
- **决策①（pnpm 预检）**：`cliInternals` 增 `platform` seam 字段（CI 仅 ubuntu，win32 分支只能 seam 测）。安装前（仅非直通路径，current/ahead/link-lane 零开销不探）探针 `pnpm --version`：posix 直 spawn（dsh 自身无 shell 语义，真 ENOENT 即定论）；win32 经 `ComSpec ?? cmd.exe /d /c` ——复刻 dsh `shell:true` 的 cmd.exe+PATHEXT 解析语义，判据是"探针找不到 ⇒ dsh 必找不到"，对 corepack/npm/standalone 三种 pnpm 装法公平；判缺仅认 posix ENOENT / win32 9009·127 无 spawnError，**不确定即放行**（探针 spawn 错、异常退出码、超时均照装，30s 探针预算），放行后的失败归因交给安装后判类。预检判缺时 dsh 不跑、profile 目录零副作用（旧路径失败时 dsh 已建 profile 骨架——副作用严格变少）。
- **决策②（失败形态扩展，D50④ 契约修订）**：成功形态不变；失败 = 一行 verdict（200 字符截断不变）+ 可选输出尾（≤6 行、每行 200 截断、与 verdict 重复行去除，两空格缩进）+ 一行 manual，仍单次 stderr 写、非零退出。`CalibrationOutcome` failed 形态增必填 `kind`（`pnpm-missing`/`timeout`/`install`/`verify`）与可选 `detail`。安装后判类序：`/pnpm not found/i`（posix 兜底，保留不退役）→ win32 `code===9009`（核心修复）→ `timedOut` → 通用 install。
- **决策③（manual 分流，受众可执行性原则）**：分流键 = `kind`（判类随失败现场归 calibrate，文案归 main）。pnpm-missing → `npm i -g pnpm (or: corepack enable pnpm), then re-run blue`；timeout → `re-run blue — downloaded packages are cached and the install resumes`；install/verify → `fix the cause and re-run blue (with a global dsh: dsh plugin …)`——裸 dsh 命令对 npm 壳新用户不可执行（问题③实测），退役出壳的失败输出，global-dsh 形态留括号逃生口。
- **决策④（预算 1200s）**：calibrate 与 swap.ts（/update）两处 parity 同调 300s → 1200s（20 分钟；慢网络实测 18 分钟）；killGrace 阶梯不动；超时 reason 派生自常数（"install timed out after 20 minutes"）。swap 侧零测试改动（`toBeGreaterThan(0)` 断言不受影响）。
...
### D58. 工具卡三改造：Read 按文件调用树 + Write/Edit 对齐彩色 Diff 面板 + XML 信封摘要（用户裁决，2026-08-26）
...
- **dogfood 发现与修复（presenter 作用域缝）**：真 profile 上 mock LLM 驱动四场景（读树/读失败/新建/改写）发现 official-model 的 presenter 解析走 `tools.get(name)` 全局视图，而 dsh 内建工具是 **agent 作用域注册**（Agent 对象即 ScopeKey）——e2e 全局注册 fake 工具掩盖了该缝，真机上 read/write 的 presenter 视图从未解析（分组与 diff 面板全部哑火）。修复：blue-app 提供 `blueToolPresentations` 窄缝（会话 commit 点绑定活动 Agent 为查看作用域，host 对象不出缝），official-model 改注入此缝，present.ts 的 ToolPresentationSource 放宽为结构类型。修复后真机四场景全绿（读树/嵌套窗口/预览/错误卡/新建面板）；mock 单一 callId（`mock-call-1`）与单工具全局参数限制下，改写 hunk 面板由 VT golden 与 e2e 覆盖、真机自然覆盖。
- **D58b 搭车四件（用户裁决，2026-08-27）**：①**grep/glob 搜索调用树**——连续搜索折叠成 `transcript-search-group`（presenter 词汇判定，与 read 同一 run 机制、互不并组）：pattern 行携带 files/matches/paths 计数（截断搜索如实标 `kept of total`）、折叠永不显示匹配文本，Ctrl-O 展开文件行+每文件 ≤3 条匹配预览与截断路径页；registry 侧 search result 同步紧凑化为计数。②**job_output 增量摘要**——上游文本本就增量且以文档化的 `[status: ...]` 收尾，摘要为 `+N lines · status X`（工具无关的尾行契约解析，非按名特判）；jobs reader 的 pending call 也声明 `kind:'read'`，故 read 组的行身份回退为 salient 参数标签（如 `job_id: 5`），无文件读取者不再隐形。③**chip 语义化**——diff 呈现的写/改卡头部 chip 从信封行数（`· 5 lines`）换为真实变更计数 `+A −D`（`toolResultChip` 从 presentation 视图树求和）。④**activity 海浪上色**——waiting/tool 相位的深海波纹（moon 帧）此前是裸文本，现在骑在 banner 品牌渐变上逐 tick 换色（空渐变回退 primary，与 composing 一致）。
- **D58c 裸 JSON 摘要（用户裁决，2026-08-27）**：presenter-less 工具（ask_user_question 的 answers 载荷等）的整段 JSON 对象/数组结果此前原样直出（`{"answers":[…]}`）。摘要器第三形状：整段可解析的 JSON 折叠为 `key: value · …` 一行（顶层与嵌套均有界：列表 3 项、对象 4 键后计数 `+N more`；单元素数组/单键对象解包直读），renderBody 的折叠门从「信封命中」放宽为「摘要有效即走」，三形状（XML 信封/增量 status 尾行/裸 JSON）统一，展开保留原文。工具无关：只认形状不认名字，非 JSON 原样透传。
