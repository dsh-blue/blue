...
- **落地（2026-08-20，S22）**：core `createFileMentionProvider` + `EditorAdapter` reopen 钩子（`mentionTokenBeforeCursor` 以 Set 判分隔符）；interaction `file-mention.ts`（探测/fallback/提取）+ editor-plus @ 分支重接；旧 fd 直调机制（`setFdRunner`/`scanFiles`/`listProjectFiles`）退役。927 tests / 63 files，test/coverage（逐文件 100%）/typecheck/lint 全绿；bundle e2e 新增 drill-down 用例（`@docs` → Enter 接受目录 → reopen 列目录内容 → 续打 `blue-arch` 预选 → Enter 接受文件不提交 → 再 Enter 以纯文本提交）。
