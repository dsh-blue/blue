...
- **决策②（dist-tag `rc`，不占 latest）**：五包只打 `rc`，latest 留给稳定线。由此**一切手动安装命令一律带 tag 后缀**：`dsh --profile blue plugin add @dsh-blue/blue@rc`——pnpm 默认解析 latest，无后缀即 "No matching version"（R4 原定命令改写依据）。
