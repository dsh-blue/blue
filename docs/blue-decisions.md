...
- **后果/边界**：最坏墙钟 6s→12s（两工具 × 列表/读两步 × 3s 全超时），罕见可接受；xclip 空剪贴板怪癖（非零退出且 stderr 空）按 no-image 分类，`failed` 留给真有诊断信息的 stderr；不可达签名匹配 `/can(?:not|'t) open display/i`（本机实测）与 `/failed to connect to a wayland server/i`（wl-clipboard #106）；clipboard-write 的 spawn runner 不合并（stdin 管道语义不同，第三消费者出现再议）；website FAQ（zh+en）按 R6a 冻结不同步（现行描述仍真）。
