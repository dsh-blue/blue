...
- **多类型探测**：两步——`wl-paste -l` / `xclip -selection clipboard -t TARGETS -o` 列类型，与准入集取交集后按准入序（png>jpeg>webp>gif）逐个 `-t <type>` 读，首个出字节者胜；交集空但存在 `image/*` 行 → unsupported；零字节/软失败试下一类型，硬失败（missing/timeout/unreachable）即刻止步。**sniff 覆写退役**：声明类型来自剪贴板自身类型表、直通 store 准入，谎报源由 `IMAGE_TYPE_MISMATCH` 拒（sniff-first 会悄悄纠正谎报而非拒绝，违背裁决精神；`?? 'image/png'` 兜底本就是死重，垃圾字节早被 INVALID_IMAGE 拒）。准入序与扩展名单一来源：attachments 导出 `ADMITTED_IMAGE_TYPES`（store 白名单从它派生）与 `EXT_BY_MEDIA_TYPE`（`pasted-image.jpg` 等文件名派生）。
...
- **后果/边界**：最坏墙钟 6s→12s（两工具 × 列表/读两步 × 3s 全超时），罕见可接受；**超时击杀用 `killSignal: 'SIGKILL'`（2026-08-22 验收期实证补记）**——Node `execFile` 默认发 SIGTERM，而 wl-clipboard trap 了 TERM 做自身清理、卡死时处理器永不返回：TERM 杀不死（Ctrl+C 同理，须 kill -9），且 exit 事件不来 → 读器 await 永挂、僵尸累积；spec 挂死假 bin 加 `trap '' TERM` 钉死该信号选择；**GNOME/mutter 不提供 zwlr data-control**，wl-clipboard 回退核心 `wl_data_device_manager` 协议、需 surface 取得键盘焦点才收到选区事件——后台/SSH 进程永远等不到（1×1 隐形窗焦点等待，实机 WAYLAND_DEBUG 轨迹实证），故 GNOME 上 wl-paste 探测恒 3s 超时、每贴一图先付 3s 延迟，xclip 经 Xwayland 桥是 GNOME 实际通路（探测序 wl-paste→xclip 维持：wlroots/KDE 原生场景仍首选）；**本机 GNOME 另有 Wayland→X 剪贴板桥接不工作的问题（freedesktop #99853/RH #1850840 缺陷族）——真桌面复制进不到 X 侧，D48 已记档的"clipboard-write.spec 本机环境性红"同根（本分支已顺手根治：EACCES 用例整替 PATH，杜绝 libuv skip-on-EACCES 漏到真二进制）**；"连续超时后会话级 memo 跳过 wl-paste"记挂起未做；xclip 空剪贴板怪癖（非零退出且 stderr 空）按 no-image 分类，`failed` 留给真有诊断信息的 stderr；不可达签名匹配 `/can(?:not|'t) open display/i`（本机实测）与 `/failed to connect to a wayland server/i`（wl-clipboard #106）；clipboard-write 的 spawn runner 不合并（stdin 管道语义不同，第三消费者出现再议）；website FAQ（zh+en）按 R6a 冻结不同步（现行描述仍真）。
- **落地（2026-08-22，S36）**：paste-image 重写（契约 + 两步探测 + 分类 + 聚合 + 六级提示 + 进行中提示）+ attachments 双导出 + spec 全量重写（插件套件六 kind 逐级断言 + 默认读器假 bin 两步矩阵，含环境门两分支与一条 3s 超时）+ clipboard-write EACCES 用例根治 + e2e 注入点跟随联合。**真机验收状态：Blue 侧逐字节忠实已两轮验证（md5 全等）、分级/进行中提示已核实；"真桌面复制 → 贴入"被本机 GNOME 的 wl→X 桥接缺陷挡在桌面层，未过。**
