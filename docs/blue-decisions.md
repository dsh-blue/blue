...
- **后果**：`/update` 入 blue 命令组（/help 列表 39 行，两处钉数断言随更）；settings `blue` 命名空间首个消费者（`installSettingsSection` + MemorySettings 真服务测试）；随批修复 `check:lib` 通配匹配器嵌套目录 bug（`lib` + `**` + `*` 型 files 条目此前只匹配一层子目录——占位符两段替换修复；嵌套 types 目录是首个触发者）；banner.spec 的 cwd 断言改深度无关（worktree 深路径暴露 compose 截断计 ANSI 的既有怪癖，另行观察未动 S35 面）。**跟发触发规则仍开放**（本机制不解决 Blue 何时跟发上游 rc）。
