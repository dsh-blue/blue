...
- **后果**：`BlueStatusEntry` 增可选 `row`/`align`（additive，谎言值钳位不崩）；footer 条目插件扩为五个（basic/cwd/git/tips/context，后两个新子路径导出）；git 探测 TTL 缓存（branch 5s / status 15s / numstat 仅脏树，惰性刷新于 render 内）；context 的窗口来源定为 adapter `resolveModel().context.contextWindow` 经 `'request/context'` 事件（v1 调研已证 `agentDefaultModel` 无元数据可取），模型切换撤回窗口即降级 `ctx N`。
