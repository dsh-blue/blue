...
- 主题插件族（各自独立 fiber，provide `blueTheme`）：`blue-theme-dark`（plain 基线默认）、`blue-theme-light`、`blue-theme-auto`（读 `blueTerminalInfo.background` 选 palette，监听 `'blue/terminal-theme-changed'` 时**dispose 自身 provider 子 fiber 并以新 palette 重新 plugin**，依赖方随之 reload）、`blue-theme-custom`（JSON 文件，zod 校验，非法 hex 丢弃回退 base——kimi `custom-theme-loader` 同款）。
- `/theme` 命令（L2）：known-themes 表静态 import 各主题入口；切换 = dispose 当前主题 fiber + `ctx.plugin(目标)`。当前 fiber 句柄由 switcher 插件模块级持有。
...
| 状态栏条目 | `ctx.blueStatus` | transcript | 注册表 + disposer | `blue-status-basic`（model·status） | 注册自定义条目；或整个替换 footer 插件 | P1 |
...
| 组合 | `cordis.patch.yml` 行 | bundle | 组合层 | 基线 5 行 | 零代码启停/重排任何 Blue 插件 | 已有 |
...
**Plain 基线 patch**（6 行）：`blue-core`、`blue-theme-dark`、`blue-transcript`、`blue-interaction`、`blue-startup`、`blue-app`。拔掉一切增强后：主屏 + dark + generic 工具呈现 + 单行状态 + 工厂编辑器 + `/quit` `/resume` + 审批/提问 overlay。
