...
| `ctx.permissionPresets` | 注册权限模式，模式 UI 自动列出 | ⛔ rc.7 不存在（仅 `approval.setPolicy` 二元 `ask`/`never`）——待上游开缝 | ⛔ |
| `ctx.sessionProjections` | 提供 todos 等投影，pane 自动呈现 | ⛔ rc.7 不存在——todos 改走 `todo/write` 会话事件自折叠（`pane-todo`） | ⛔ |
