...
`check:pack` creates `.artifacts/pack/index.json` and eleven tarballs. It runs `publint`, AreTheTypesWrong, manifest checks, bin checks, dependency protocol checks, shrinkwrap checks, and package-size budgets. Release automation consumes those exact tarballs; it never rebuilds a second copy.
