...
| 12 | 思维链与活动指示 | reasoning 永远全量内联可见；activity pane 单态恒显、与 thinking 双转 | live spinner + 尾部 2 行滚动，完成自动折叠 2 行 + ctrl+o；活动模式机（waiting/tool=moon、thinking 空、composing=braille working...） | S17 |
...
**Dogfood 第二轮（2026-08-20，用户两项反馈）**：(1) **`/theme` Up 召回编辑器错乱**——根因：pi-tui 聚焦编辑器在光标前发射零宽 APC 硬件光标标记 `\x1b_pi:c\x07`（IME 定位用，渲染前由 renderer 剥离），Blue 的 chrome 纯函数只跳过 SGR 序列，标记被当成 7 个可见字符；Up 召回把光标放在文本首字符上（inverse 字形而非 inverse 空格），`injectGhostHint` 的"光标后全是 padding"守卫整体跳过，`visibleIndexToRaw(4+textLength)` 落进标记中间，影子文本+空格**吃掉召回文本与光标**。kimi 解法核实（`custom-editor.ts` `computeArgumentHint`）：**源头光标门禁**——光标必须在第一行行尾才计算 hint（`line !== 0` 或 `col !== currentLine.length` 即 undefined），行数学的 raw-index 路径在 kimi 恒不可达。Blue 补齐同一门禁：core `EditorAdapter` 增 `cursorAtInputEnd()`（读 pi-tui `getCursor()`/`getLines()`），ghost 注入前判定；红验证过（门禁关闭时新 spec 失败），多行光标分支同补。kimi 的 `stripSgr` 同样只处理 SGR——不加固行数学、只做门禁是两侧一致的选择。(2) **对话框打开时编辑器应隐藏**（D30，修订 D26 锚定手法）——浮层 `offsetY: -2` 只让出 footer，编辑器框在面板与 footer 之间露出（e2e 帧 dump 证实）；kimi 全部对话框走 `mountEditorReplacement`（editorContainer 换子，编辑器离树但状态存活）。移植：`editor-instance` 开 `EditorSlotSwap` 缝，blue-input 实现栈式替换（面板入栈摘 editor+hint、弹空恢复还焦，fiber 卸载随树下架未关面板），四个调用点（/sessions、/help、approval、questionnaire）改走替换、浮层锚定参数退役；替换面板在 dock 真实占位、其下只有 footer，framePanel 观感不变。已知边界转 S17：对话框挂起时 pane-activity 仍悬在面板上方、btw 拼接指向离树编辑器（kimi 此时 activity pane 亦隐藏）——**两条均已在 S17 闭合**（`'blue/editor-slot-swapped'` 事件：pane-activity hidden 态 + pane-btw 拼接撤回/重申，见 §7 S17 落地注）。e2e 两条新锚：面板打开时编辑器框 SGR（`38;2;90;90;90`）整帧缺席、Escape 后回归；/theme 召回帧含 `theme` 且无 `[dark|light`。**连带测试考古**：banner 真实 tips 使 e2e 既有 `/help`/`/sessions` waitFor 锚被横幅文案**瞬时假满足**（同步语义破坏，全套负载下才显形）——锚改下拉专属文本（`→ /sessions` 指针行、`Show available commands` 描述行）；召回用例收尾补 `clearDraft()`（模块级草稿 stash 不随 tree dispose 清除，泄漏进下一用例编辑器把 follow-up 变斜杠命令）。824 tests / 58 files，coverage（逐文件 100%）/typecheck/lint 全绿。

**Dogfood 第三轮（2026-08-20）**：`/theme <name>` 带参提交后不进编辑框历史——`blue-input` inject `blueTheme`，换装重建 fiber 时 **pi-tui Editor 的组件内历史随旧组件销毁**（无参 `/theme` 只列清单不换装故无恙；kimi 无此问题——其编辑器是跨主题存活的单例组件，主题经模块级 `currentTheme` 渲染期读取）。修复沿用草稿 stash 同一先例（同一根因、同一模块存活语义）：`draft-stash` 增 `stashHistory`/`getStashedHistory`（新到旧序，pi-tui `unshift` 语义），`BlueEditor` 契约 additive 增 `getHistory()`（pi-tui `history` 字段 d.ts 私有——单点结构 cast 经 `unknown`，WrappingSelectList 先例，components spec 钉 0.84.2）；提交路径镜像全量历史（prompt 侧在 `addToHistory` 后、bash 侧在 editor-plus 自己的提交分支同步镜像），重挂载按序倒放重建。fake 编辑器历史语义对齐真实契约（`unshift` + 最近项去重，原为 push）。**测试考古**：pi-tui `getSuggestions` 异步——测试同一同步突发内输入字符+Enter 时，confirm 会把仍在飞的旧轮选中项 apply 进 buffer（'/theme light' 被 '/theme' 覆写成列清单），e2e 在 Enter 前 `waitForRender` 让空结果轮先结算（真实打字有人类节奏不触发）；slash token 加粗上色使 '/theme light' 字面量在原始帧中被 SGR 切断，召回锚经 `stripSgr` 断言；编辑器 stash（草稿+历史）为模块态，各 spec afterEach 与 e2e afterEach 补隔离重置。826 tests / 58 files，coverage（逐文件 100%）/typecheck/lint 全绿。
...
**落地（2026-08-20）**：按上述四 bullet 全量落地，含三处裁剪与两处结构裁决——(1) **fold 签名升维**：`apply` 返回 `readonly FoldUpdate[]`（挂载序），因为 `assistant/message` 在无流式 step（derived history / 快照回放）一次产出 thinking + assistant 两条目，单 update 装不下；流式路径仍是单元素数组，e2e/fold.spec 断言随之 `[0]` 化。(2) **finalize 不走显式 `finalize()` 而走条目变异**：Blue 组件逐渲染读 item（`AssistantMessageComponent` 先例），thinking 组件的 spinner interval 在首个观察到 `streaming === false` 的 tick 自行退役——快照回放同步构造成千上百个 once-live 块，无一个 tick 真正触发；`dispose()` 由挂载器经 `retireEntry` 鸭子类型调用（淘汰/换会话/窗口逐出停表）。(3) **重试明细行整行裁剪**：rc.7 事件面（turn/step 边界 + chunk + tool + request/todo）无任何 retry 记录，`RETRY_DETAIL_MAX_CHARS` 不设死常量，重试读作普通 waiting。(4) **phase 归属**：`StreamingPhaseTracker`（`src/phase.ts` 纯状态机）即"fold 的 streaming 阶段"的独立投影，pane-activity 以同源过滤的事件流喂它、attach 时先以持久化快照播种（恢复中的会话立即落对 phase）——不立第二套平行推导；映射随 kimi：`assistant/message` 保持残相（thinking/composing 不因终稿翻转）、`turn/end` → idle、tool-call-delta → tool、空白 reasoning 不翻 thinking（`handleThinkingDelta` 守卫）。(5) **对话框挂起收口**：core Events 新增 `'blue/editor-slot-swapped'`（blue-input 面板栈 空↔占用 迁移时发，嵌套不发、带面板卸载补发 false），pane-activity hidden 态与 pane-btw 拼接重申（S16 dogfood 两条已知边界就此闭合）。其余：moon/braille 帧常量落 `src/spinners.ts`（80/120ms），月亮行宽经活读 `visibleWidth` 实测；tip 取 S15 SWRR 序列下一位（loading 种类变化时取新、种类消失不发）；idle 占位棘轮（`everActive`）随 kimi——冷启动零行、首次活动后恒一行；pane-activity 自注入 `blueComponents`（dock 序钉位等价，bundle 行的 row-level pin 撤销）；混排边界随 kimi 采纳：同 step 内 reasoning 晚于正文流出时 thinking 块按到达序挂载（DeepSeek 流序 reasoning 恒先，回放路径由 fold 保证 thinking 居先）。e2e 五条新增/改写：waiting moon（`hang-silent` 脚本）、thinking 空窗 + live 尾窗（`hang-reasoning`）、finalize 折叠 + ctrl+o 往返、对话框挂起隐藏/回归、resume 全 finalized（`reasoningResponse` 脚本助手）。859 tests / 60 files，coverage（逐文件 100%）/typecheck/lint 全绿。**Dogfood 四轮裁定（同日）**：(1) 首轮删 composing `working…` 行——当时的判断是光标已覆盖该信号；(2) 二轮（用户要求再研 kimi、尽量对齐）恢复并补全对齐：核实 kimi `assistant-message.ts` **无流式光标**，pane spinner 是 composing 的活跃信号；恢复 braille `working...` 行（ASCII 标签、primary 帧 + 默认色标签——`MoonLoader` 的 `colorFn` 只涂帧——+ tip 随行，宽度不足先弃 tip 再弃行），tip 轮换恢复 moon/composing 双种类（种类变化取新），idle 占位改 kimi `Spacer(1)` **恒显**（去棘轮）；(3) 三轮（用户裁定）**退役 Blue 流式 `▌` 光标**——kimi 助手块无光标，composing 信号全归 pane；assistant 条目的 `streaming` 标志随光标退役一并移除（其唯一消费者），thinking 条目的保留；(4) 四轮（用户实机反馈正文形态两问题）：**分割线长度**——根因 pi-tui markdown `hr = '─'.repeat(Math.min(width, 80))` 硬 80 上限（kimi 同样继承，但其正文经 `MESSAGE_INDENT` 收窄、80 列终端下规则恰与正文同宽；Blue 在宽终端上规则只铺左侧）；core `MarkdownAdapter` 后处理：以主题 hr 输出的精确串匹配（容忍行 padding）重绘为满渲染宽，码块等高亮行自带 SGR 不误伤；**正文边距**——S18 助手半边提前落地：`● `（`text` 色）+ `MESSAGE_INDENT = '  '` 两列缩进 + markdown 按内容宽渲染（分割线随内容宽对齐正文）；(5) 五轮（用户实机反馈两问题）：**注入上下文隐藏**——D28/S19 拉前落地：fold `user/message` 按 `source.kind !== 'user'` 零呈现（实现按真实契约 kind 分拣，见 S19 落地注）；**右侧边距**——D29/S21 拉前落地：core `GutterComponent` 挂载层统一包装（transcript 条目/banner/四 pane/footer 各内收 1 列，编辑框贴 0 列），全量布局锚 reflow 一次性完成（banner 满宽断言改内收、pane 规格经 strip/unwrapped 助手去沟）。计数 868/61。(6) 六轮（用户实机反馈两问题，均**记录不实现**——按用户"非本阶段不做"规则）：**工具展示缺口**——write 工具只以 step-summary 折叠行出现（S7 kimi 同构设计，非 bug；呈现缺口归 S20 卡片 + S18 措辞，注记入 S20 节）；**滚动混乱**——输出中移动终端滚动条导致会话流上下乱跳：根因 Blue 跑 `TuiMainScreen`（主屏模式，滚动缓冲即终端原生 scrollback），pi-tui 差分渲染在内部记账 `previousViewportTop`/`hardwareCursorRow`，用户移动滚动条使终端实际位置与记账脱节，后续差分按错误行偏移写屏；主屏模式终端滚动位置不可查询，此冲突固有。kimi 的答案即 alt-screen 全屏（渲染器自持滚动，env 门控实验特性）——Blue 侧归 **L0 alt-screen 项目**（p2-visual §8 已判"本文档范围外、另行立项"，roadmap P1/P2 两处既有条目），滚动流畅验收（P2）以此为前提。

### S18 — 用户回显 + 助手消息 chrome（✅ 已落地（2026-08-20））

- transcript：`UserMessageComponent` —— `✨ `（USER_MESSAGE_BULLET 入符号常量）`boldFg(roleUser)` + **全文 boldFg(roleUser)**；续行以 bullet 可见宽对齐；图片缩进同宽。`AssistantMessageComponent` —— 行首空行 + `● `（`text` 色）bullet + 正文 `MESSAGE_INDENT = '  '`（2 列）缩进（✅ 助手半边已随 S17 dogfood 提前落地——正文边距四轮裁定；本步剩余：用户回显 ✨ + step-summary 措辞）。step-summary 措辞顺带对齐 kimi（`… thinking N times, called M tools`，dogfood 定——S17 期间实测样本 `… step 1 · write ×1` 即此面）。
...
**落地注（2026-08-20）**：用户半边全量落地——`UserMessageComponent` 换 kimi user chrome：`\x1b[1m` bold 包裹 `roleUser` palette 色的 `✨ ` bullet（BOLD_OPEN/CLOSE 本地常量，pane-btw/todo 先例；kimi `boldFg` 组合的 Blue 等价——v2 主题契约 28 token 无 bold 函数，不为一步加签名），全文按换行后的每行独立 `bold(roleUser(line))`（kimi 是整体染色后过 ANSI-aware wrap，逐行染色输出同形态的逐行完整 span，spec 等价）；续行与图片行缩进 `visibleWidth('✨ ')` 空格（fake 下 2 列，实机随 pi-tui 度量）；图片改 `contentWidth` 渲染（原全宽）。**step-summary 措辞对齐落地**：foldStep 折叠面扩到 thinking 条目（一步至多一块 reasoning）——summary 行改 kimi 词汇 `… step N · thinking X times, call Y tools`（两段逗号连接、零值段省略、沿用 kimi 无条件复数 `1 times`/`1 tools`；`step N ·` 前缀保留——Blue 折叠按步进行，一轮可有数行摘要，前缀即步号消歧）；fold 同步剪枝：被折叠的仍 streaming 的 thinking 条目清 `streamingThinking`/`pendingReasoning`（`evictThrough` 对称，live 流中不可达——上一步 `assistant/message` 恒先于下一步 `step/start`）。e2e/spec 锚全面重铺：theme-swap 断言改 `\x1b[1m`+light roleUser `✨ ` 前缀、summary 锚 `… step N · call Y tools`、`❯` 全部退役（/sessions 指针不同表面不受影响）。**dogfood 待定项**：措辞的最终形态（`call 1 tools` 单数观感）与 `⎿` 无关，留 dogfood 裁定。871 tests / 61 files，test/coverage（逐文件 100%）/typecheck/lint 全绿。

### S19 — 注入上下文默认隐藏（D28）（✅ 已随 S17 dogfood 拉前落地）

- transcript fold：`case 'user/message'` 读 `event.data.source`——有 source（合成：agent-instructions/plugin/notice/relay/recall 等 ContextFormed 词汇）→ **不产出条目**（零呈现、零占位）；`source === undefined` → 人类输入照常。快照回放同规则（D16）。**落地注（2026-08-20，五轮裁定）**：实现按真实契约 `source.kind !== 'user'` → 零呈现——`Message.source` 是必填判别联合（`user`/`plugin`/`model`/`tool`），`kind: 'user'` 即人类；D28 文本的"source === undefined"是裁决时的粗略表述，实际以 kind 分拣（merge 扩展的未知 kind 按非人类归隐藏）。e2e 两条：live 注入零呈现、resume 快照一致（`surfaceOp: 'append'` 直插会话日志）。
...
### S20 — 工具卡 kimi 折叠（✅ 已落地（2026-08-20）：前半卡片头/预览/ctrl+o 范围 + 二轮 dogfood 折叠保留窗口；后半 Read 分组 + shell 呈现）
...
**落地注（2026-08-20，前半轮 + 二轮 dogfood 修正）**：按拆期预案先行前半——卡片头/预览/ctrl+o 范围。**卡片头**：`ToolCallComponent` 三态化——运行**实心 `● `**（`text` 色，消空心→实心闪烁，kimi 裁定采纳）、成功 `✓ `（success）、失败 `✗ `（error）；标签 `Using/Used ToolName (keyArg)`——动词本色、工具名 bold primary（`\x1b[1m` 包裹 S18 先例）、keyArg dim 括号（白名单 `file_path`/`command`/`pattern` 优先、未命中取首个 ≤60 字符的字符串参数兜底、ellipsize 压 60）；`bash` 纯标签 `Running/Ran a command`（命令归正文；Blue 的 shell 工具走 terminal intent，此分支是 presenter 缺失回退）；完成态 dim chip ` · N lines`（非空行计数，失败 error 色，空结果抑制）。**MCP dim 后缀记录不实现**——rc.7 无任何 MCP 表面（dsh-mcp 不存在），缝哲学：不为不存在的消费者开分支。**预览层级**：折叠态 = 全文按内容宽 wrap 后**前 3 个视觉行**（wrap-aware，kimi `TruncatedOutputComponent` 语义）+ dim hint `... (N more lines, M total, ctrl+o to expand)`（N/M 为 wrap 后视觉行数，hint 随宽度截断）；`⎿` 连接符退役、两列缩进直排（kimi 无此符号）；展开态 = 全部 wrap 行。**intent 上限对齐**：diff 折叠 12→10（`COMMAND_PREVIEW_LINES`）、terminal 折叠 10→3（`RESULT_PREVIEW_LINES`，kimi Bash 结果预览同数），两张卡截断行统一换 kimi hint 措辞。**ctrl+o 范围**：`CollapseToggle` 弃组件 Set，改携会话挂载序 `entries`——按用户条目边界定位 `(totalTurns − 3)` 轮起点，起点之后的组件才翻、之前的强制折叠（kimi `toggleToolOutputExpansion` 位置判定）；会话切换重置语义不变，新挂载组件恒在最末轮自然入范围。**bundle e2e**：Ctrl-O 用例加新卡锚（`✓ Used long-output`/` · 1 line`/hint）；ctrl+o 3-turn 范围判定由 plugin.spec 四级 turn 用例承载（e2e mock 单请求整脚本，无法脚本化四轮）。**二轮 dogfood 裁定（同日，用户实机）**：(1) `⎿` 连接符**退役确认**（不再回炉）；(2) `view.title` **暂不展示**——rc.7 `GenericCallView.title` 是 harness 设计的卡片头文本，本期维持 kimi 形态（ToolName 行）；记录为**未来重设计候选**：若 Blue 要做自家工具卡的标题面（而非纯 kimi 复刻），title 是第一个接入点；(3) **折叠保留窗口对齐 kimi**（用户问"工具调用基本都被折叠掉了，这符合 kimi 逻辑吗"——不符合：kimi `TRANSCRIPT_KEEP_RECENT_STEPS = 30`，一轮内最近 30 个 step 的卡片全可见，折叠只是超长 turn 的保险阀；Blue S7 的"下一个 step/start 折叠上一个 step"把正常多步 turn 折叠得只剩最后一步——S17 六轮注记"kimi 同构"的判断有误，特此更正）——fold 改**滑动保留窗口**：`DEFAULT_RECENT_STEPS_RETENTION = 30`（`setRecentStepsRetention` 测试钉针），新 step/start 只折叠滑出窗口的 step；一轮最后若干 step 恒不折（无后续 step/start 到达）。perf 界随之重算（每轮 8 组件：user + 3×2 工具 + assistant）。879 tests / 61 files，test/coverage（逐文件 100%）/typecheck/lint 全绿。

**落地注（2026-08-20，后半轮）**：按拆期预案收尾后半——Read 分组 + shell 呈现。**Read 分组**（`src/read-group.ts` + mounter）：`ReadGroupComponent` kimi `read-group` 移植——同 step **连续** ≥2 个 Read 折叠为一组（kimi 连续性规则：组间插入任何其他工具即断链，Blue 由 mounter 的"上一条目检查"表达；kimi 端同规则见 `streaming-ui.ts` `tryAttachReadToolCall`——非 Read 重置 `_pendingReadGroup`）；成员状态留在各自 fold item（kimi"state stays in each card"的 Blue 等价——组每渲染直接读 member item，tool/result 事件经既有 requestRender 路径推动重绘，无需 kimi 的 snapshot listener）；组头三态——运行 `● `（text）+ bold primary `Reading N files…`、完成 `✓ ` + `Read N files · L lines`（dim 行数 + error ` · F failed` 尾部）、全失败 `✗ ` + bold error `Read N files · failed`；树体 `  ├─/└─ path · N lines`（` · reading…`/` · failed` 尾），无路径成员只入头不入树；无 `setExpanded`（kimi 组不展开，Ctrl-O 跳过）；组的簿记 item = 首成员——step 折叠与窗口逐出经首成员匹配整组退役。**Read 识别走 view 契约**（非 kimi 的名字匹配）：`present.ts` `isReadItem` 认 rc.7 的 pending `GenericCallView`（`kind: 'read'`）与完成态 `card: 'read'` `ReadResultView`——Blue 仓库看不到 harness 的具体工具集，文档化 view 是唯一稳定信号；首个 Read 先按普通卡挂载、第二个到达时卡片退役改组（kimi solo→group 升级同构），第三个起 attach。**shell 呈现**：terminal 卡命令 chrome 对齐 `shell-execution.ts`——`$ ` shellMode + 命令体 muted（kimi textDim → Blue muted 映射）、多行命令续行 4 列缩进、折叠态命令预览 10 行上限（`COMMAND_PREVIEW_LINES`，展开无上限、无 hint——kimi 命令预览原样切不补行）；exit badge 随首行、零出口静默、信号 warning（S7 语义保持）；输出维持 textMuted 一层更暗（两档分离）。generic 卡的 bash 回退分支同步：命令进正文预览（同 chrome，pending 即显示，10 行折叠上限），presenter 缺失的 shell 也能看到跑了什么。**bundle e2e**：mock 单请求单调用结构无法自然产同 step 双 Read——新增 `twoToolCallsResponse` 本地辅助（双 tool-call block、index 0/1、单 finish），分组用例走真实 agent-loop。898 tests / 61 files，test/coverage（逐文件 100%）/typecheck/lint 全绿。**S10-S21 会话流对齐五期全部收官**。

**落地注（2026-08-20，三轮 dogfood——用户实机对比 kimi Write/Read 卡）**：(1) **Write/diff 卡 kimi 对齐**：kimi 的 Write 是"行号文件预览"（`    N  ` dim 行号 + 正文，`Used Write (path) · N lines` 头），Edit 才是 diff 行；Blue 原实现把两者都画成统一 diff（title 行 + `path (+A −R)` 行 + `+` 全加行）。DiffCardComponent 重构：共享 kimi 卡头（三态 bullet + `Using/Used ToolName (keyArg)` bold primary + 完成 chip——纯 create 调用 ` · N lines`（kimi Write chip，尾随换行不计）、含 edit 块 ` · +A -R`（kimi Edit chip，零值省略、双双为零时无 chip）；正文按 oldText 分流——`oldText === null`（create/覆写 = harness Write）→ **行号文件预览**（dim `    N  ` 编号 + 正文、尾随空行保留——kimi `highlightLines` 形态，空内容零行；折叠 10 行 + kimi hint、展开无上限），edit 块 → 统一 diff 行（折叠 10 / 展开 200 + hint）；旧 title/path-count 行退役（path 已在 keyArg，view.title 维持"暂不展示"裁定）；语言高亮不做（kimi 按扩展名 highlight，Blue 无对应基建——记录不实现）；kimi 的 `Approved: Writing …` 审批结果行无 Blue 数据源（approval 交互走 Blue 对话框），记录不实现。(2) **Read 默认折叠**：kimi Read 卡 = `Used Read (path) · N lines` 头 + 零正文（内容 ctrl+o 才展开）；Blue generic 卡原默认 3 行预览。`ToolCallComponent` 加 `isReadItem` 折叠——Read 卡折叠态仅头+chip，展开全量（行号 ReadResultView 视图留作未来项）。(3) **纯函数上移**：`ellipsize` 与 keyArg 白名单提取移入 `present.ts`（`extractKeyArgument` 供 generic/diff 两卡共享），fold 改 re-export——避免 present↔fold 循环依赖。904 tests / 61 files，test/coverage（逐文件 100%）/typecheck/lint 全绿。
...
**落地注（2026-08-20，五轮裁定拉前——用户实机反馈"右侧还是没有边距"）**：core 新增 `GutterComponent`（`src/gutter.ts`，kimi `GutterContainer` 等价：child 按 `width-2n` 渲染 + `padColumns` 左垫 n 列，样式不动、invalidate 透传）；挂载层统一包装全部内收面（transcript 条目含 step-summary、banner、四 pane、footer），编辑框/对话框/overlay 贴满宽不动。reflow 一次性完成：banner 满宽断言改内收锚（`columns - 1`）、footer 行首 gutter 锚、pane 规格经 strip/unwrapped 助手去沟、plugin.spec 新增 mounter gutter 直断言。S13 推迟注记与 D29"待落地"随此移除。
