...
| S36 | paste-image 失败分级提示 + 多类型探测（**D49**，2026-08-22 用户裁决：提示缺失而非代装/绕过；SSH 无显示环境实测根因）：读器契约 `ClipboardImageResult` 六级失败分类（missing-tool/unreachable/timeout/failed+detail/unsupported/no-image 各报所缺）+ 两步探测（`wl-paste -l`/`xclip TARGETS` 列类型 → 准入序读字节，JPEG 源不再误报）+ 聚合 rank-map 与 unreachable/missing 环境门 + sniff 覆写退役（store 交叉校验即真相）+ `ADMITTED_IMAGE_TYPES`/`EXT_BY_MEDIA_TYPE` 单源导出 + `pasting image...` 进行中提示（验收二轮反馈）+ 超时击杀 SIGKILL（wl-clipboard trap TERM 实证） | 0.5d | 无硬前置（interaction 独立面，worktree `worktree-s36-paste`；目录名刻意短——transcript banner spec 的 Directory 行断言对缩短后 cwd >85 的路径敏感，S35 的 23 字符名已压线） |
