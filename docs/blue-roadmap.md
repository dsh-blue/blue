...
| S28 | 配置与生态（**partial ✅ 2026-08-21**：/tools + /preset 已落地，含**薄宿主迁移**——bundle patch 照 web-app 官方清单 disable dsh-base 23 行 agent 面行 + agent-presets 行 + blue-app 建 agent 挂预设（resume 折 `agent-preset/selected` 事件重建组合，详 D37）；/settings ✅ 已落地（2026-08-25，分支 p2/s38-settings，⏳ 待人工验收）、/reload /tasks 顺延；**/mcp 提前为 S34 落地（2026-08-22，用户裁决）**） | 3d+ | S27' |
| ✅ S29 | 技能管线（✅ 已落地 2026-08-21）：`#` 提示符（editor-plus `#` 补全分支复用 slash fuzzy + pi-tui 非 slash 前缀 Enter 接受不提交 + applyCompletion 行中替换带尾随空格）+ 提交重写（`#name`→`/name` 走 followup/steer 分支，`skills-catalog.ts` 沉降缓存镜像上游手势边界）+ `/skills` 只读 InfoPanel（按来源层分节）；**改判（2026-08-21 用户裁决）**：原「前置修复 = input 层未注册 `/xxx` miss 回退 `agent.followup`」**不做**——行首斜杠维持严格命令域（未注册即报错），技能调用经 `#` 走 followup 分支；btw 侧栏提交接重写与 slash 下拉技能双源一并记挂起；**验收注记（2026-08-21）**：手势调用对用户不可见——会话流回显的是重写后的 `/name`（键入的 `#name` 仅存编辑器历史）、skill-invocation 注入按 D28 零行渲染，技能加载无任何前端提示，体验不理想；最小可视化（用户行回显键入原文 / 一行 dim 注入标记，系 D28 例外）记挂起发版后 | 2d | S27'（dev 可‖S28，合并串行） |
| ✅ S30 | 终端小件批（**拆步① ✅ 2026-08-22**：会话标题全自动展示 + 模型热键——用户裁决 `/title` 命令 🚫 不做（标题全自动无手动入口）；节奏 = **all-prompts**（bundle patch disable first-prompt-llm + 换 dsh-session-title-all-prompts-llm，config 照抄 base，CC 式随会话流更新、歪标题下条消息自纠）；展示两路互补 = **OSC 0 终端标题**（core `terminal-escape.ts` 纯转义 `buildTitleOsc0`/`sanitizeTitleText` + 32 码点截断 + `BlueTerminalRuntime/blueScreen.setTitle` 缝，interaction `terminal-title.ts` 插件 fold 驱动、无标题回退 `blue`、去重发射、tmux 不做 DCS）+ **footer 第 2 行左簇条目**（transcript `status-title.ts` priority 12 muted 档，无标题 render '' 自动消失，banner 行 🚫 裁决不做）；**模型热键 alt+m**（免清空切换：当前 provider 模型列表循环直切、session-only 提交不写默认、effort 不随行= /model \<id\> parity、60s TTL 缓存 listModels、bash 模式同生效；键位已过 keymap 冲突检测，R2 键位冻结前最后一键）；**拆步② ✅ 2026-08-22 实现全绿（随用户三项追加裁决扩为三合一，详 D45+D47）**：`/sessions` cwd 圈定（归一化精确匹配，无子树扩散）+ 标题命名行（`sessionQuery.readTitleSnapshots` 批量读、newest-K=100 护栏、缺席/失败降级 id 形态、cwd 退役让宽）+ type-to-filter（`SelectListPanel` opt-in `filter` + `filterText` 缝，ModelPanel 同款交互，Esc 先清后取消；跨页搜索仍挂起）+ **退出遗言**（用户裁决：退出打印会话 id 与恢复命令，app 层 dispose arm + `process 'exit'` flush，详 D47）） | 1.5d | S29 |
...
| ✅ S32 | 大粘贴折叠（**2026-08-21 重定范围**：编辑器半**原生已有**，无需移植——pi-tui Editor 内置 >10 行/>1000 字符折叠为 `[paste #N +M lines]` 标记、`pastes` Map 存全文、`submitValue` 提交前自动 `expandPasteMarkers`（模型收全文已核实）、`getExpandedText()` 公开；此前"需移植 kimi paste-burst"判定系按 fork 文件名查证失误）。**transcript 侧 ✅ 2026-08-22 实现全绿（D46，两项用户裁决：形态 = 3 行预览 + 提示行、阈值 = 同编辑器 >10 行/>1000 字符）**：长用户消息折叠（组件局部原始度量、ctrl+o 同权展开、resize 不重折）；已知小疣：历史召回为展开全文（大粘贴 Up 召回整段进编辑器，记边界不修） | 0.5-1d | S33 |
...
| 步 | 范围 | 档 | 前置 |
|---|---|---|---|
| ✅ S37 | `blue` 壳包 `@dsh-blue/blue-cli`（**D50④**）：bin 名 `blue`、嵌套钉版 `@deepseek-ai/dsh` 作运行时宿主（A 方案——用户全局有无 dsh 皆可，版本恒与测试线一致）、托管校准（启动读 profile 清单，bundle 版本不一致才 `plugin add`，一致零开销；升级 = 重装壳一条命令，**无 `blue upgrade`**）、`-V` 一行三段自答（壳/Blue/harness 线）、`--patch`/`--dump-config`/`plugin` 子命令翻译透传、startup.ts `program.name` 文案改 `blue`、version.spec 扩 manifest、website quickstart 双写（`blue` 为主、dsh 命令并列）；bootstrap 失败一行报错指路手动命令；发版走 release.yml 同管道。**✅ 已落地 2026-08-23**：`packages/cli`（translate/nested/calibrate/main 四模块 + internals 缝层 + `v8 ignore` 守护的 bin 入口，45 测逐文件 100%）；参数语义核实——`--patch`/`--dump-config` 顶层直通、`plugin` 子命令翻译（`--profile` 前缀 vs 后置两种位置）、用户 `--profile` 一律吞掉、`-V` 先于 `--` 生效；校准跳过 `link:`/`file:` 泳道（三泳道规则）；品牌缝 = `BLUE_LAUNCHER=blue` 环境变量（startup 帮助示例 + 退出遗言 resume 命令两处消费）；version.spec 扩至八 manifest + 壳的裸 dsh 钉版断言；release.yml verify/hygiene 双循环加 blue-cli（七包）；**副作用与解法**：dsh 宿主经 cli 隔离依赖入 .pnpm → loader 的 internal importer（node-addon-require-builtin）在 Node 24 激活 → /mcp e2e 裸名行改从临时 profile baseUrl 解析——e2e-boot 给临时目录链 dsh-mcp-client（生产语义等价：真 profile 的 node_modules 就在 cordis.yml 旁）；updater 核心不 import（避免给插件包开 exports 子路径，S30 事故族）——壳自含 ~30 行 profile 读取，D52「复用」按此落地记档；**随批三补丁（dsh-TUI 对照复盘产物）**：校准方向守卫（profile 经 /update 领先壳时不降级、提示重装壳前进；内联 semver 比较器）+ `ERR_PNPM_ADDING_TO_ROOT` 带 `-w` 单次重试 + pnpm 缺失报错带 `corepack enable pnpm` 建议；pnpm 前置勘误（两条安装路径都需要，guide 已改）；D53 跟发定档与锁步退役条件随批记档 | 0.5-1d | R3 发包（rc.1 五包须已存在） |
| ✅ S38 | 用户侧安全更新（**D52**）：`blue-update-check` 启动检查（loader settle 后一次 `npm view`/registry 元数据、24h 缓存、静默失败、settings `blue` 命名空间 `updateCheck`/`updateChannel` 可关）+ `/update` 命令（六道预检：link 污染/组一致性/目标存在/D51 下限/宿主线/冷却期 ETA → 打字确认 → 快照 → 单事务精确版本安装 → 装后组校验 → 冒烟 A 导入扫描 + 冒烟 B pipe-stdio boot → 失败自动回滚全组重装；`/update <version>` 钉目标、bare 即只读检查）；滚动区两行通知组件 + 编辑器 hint（banner 不动）；`updater/` 全收 `blue-interaction`、S37 落地后壳复用核心。**✅ 已落地（2026-08-22 实现，PR #37；2026-08-23 随 rc.3/rc.4 发货，真机升级/降级/回滚矩阵全过，详 D52 与 docs/history/blue-update-dogfood-s38.md）** | 1.5-2d | R3/R4（包已在 npm） |
| ⏳ S39 | paste-image 跨平台（**D55**）：win32 单 spawn PowerShell 探针（WinForms + staging 目录经 `BLUE_PASTE_STAGE` 交接 + kind/order/退出码协议 + FileDropList 批次，10s 预算）与 darwin osascript 探针（`clipboard info` 列类目 → PNGf/JPEG/GIF 直读 → TIFF+sips 转换 → Finder furl 批次，5s 预算）；复制文件批次三平台对等（`readCopiedPaths` 共享预检）；win32 `alt+v`+`ctrl+v` 双绑定；`backend` 配置仅 Linux 生效；PATH 注入 node 假工具全分支覆盖（脚本字面量 pin）。**⏳ 待人工验收**（Windows 10/11 + macOS 真机矩阵，`blue-s39-paste` profile，通过前勿并） | 1.5-2d | 独立（S38 后任意时点） |
| ⏳ S40 | 壳 bootstrap 治本（**D56** 的彼岸）：dsh 依赖树随壳 tarball 分发（bundle JS 依赖、平台二进制保持 optional——sharp/node-pty/koffi/ripgrep 无法 bundle，457 → ~30 包），消除首跑运行时 pnpm/profile 装配——首跑秒级、无 pnpm 前置、无 1200s 预算；D56 的预检与 win32 9009 判类随 pnpm 依赖一并退役 | 3-5d（含独立设计轮） | D56 落地 + 上游 win32 shell issue 回音；牵连 version.spec 壳依赖专测、check:lib、smoke:happy 的 hoisting 副作用、平台二进制分发矩阵 |
...
| /injections | 注入上下文显隐开关（原 S27' 范围项：fold.ts `source.kind` 分拣可开关 + dsh-settings 'blue' 命名空间持久） | 2026-08-21 用户裁决：注入上下文维持 D28/S19 默认隐藏，不开开关 | 真实需求出现再议（'blue' 命名空间已由 /settings 落地接管注册——2026-08-25；届时本项只是又一消费面） |
