...
| ✅ S29 | 技能管线（✅ 已落地 2026-08-21）：`#` 提示符（editor-plus `#` 补全分支复用 slash fuzzy + pi-tui 非 slash 前缀 Enter 接受不提交 + applyCompletion 行中替换带尾随空格）+ 提交重写（`#name`→`/name` 走 followup/steer 分支，`skills-catalog.ts` 沉降缓存镜像上游手势边界）+ `/skills` 只读 InfoPanel（按来源层分节）；**改判（2026-08-21 用户裁决）**：原「前置修复 = input 层未注册 `/xxx` miss 回退 `agent.followup`」**不做**——行首斜杠维持严格命令域（未注册即报错），技能调用经 `#` 走 followup 分支；btw 侧栏提交接重写与 slash 下拉技能双源一并记挂起 | 2d | S27'（dev 可‖S28，合并串行） |
