...
| R4 | 安装路径验证：干净环境（临时 DSH_HOME）`dsh plugin --profile blue add @dsh-blue/blue@rc`（**一律 @rc 后缀，D50**：五包只打 rc 不占 latest，pnpm 默认解析 latest、无后缀即 404）→ 启动冒烟；guide 补 registry 安装路径；遇"装不上"先查 pnpm 11 minimumReleaseAge 冷却期（R1 同源坑） | 0.5d | G3 |
