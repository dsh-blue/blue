...
| `/context`（原 `/usage`） | ✅（名为 /usage） | — | ✅ | — | ✅ 已发货（S25 落地 2026-08-21，**2026-08-21 用户裁决更名 `/context`**——CC 的 /context 本就是上下文占用面板；同 `InfoPanel`——四桶 + 总数 + 占用条；`sessionProjections.snapshot` 读投影（跨 resume 重放正确），降级主机走 `usage.ts` 纯折，§4.2） |
...
| `/context`（注入显隐） | — | — | ✅ | — | **S27** ⚠️ 待建且**需另取名**（注入上下文显隐开关，D28/S19 语义反向——原拟名 `/context` 已被 S25 的上下文占用面板占用：CC 的 /context 实为占用面板而非显隐开关，2026-08-21 用户裁决按 CC 语义归位；实现时另选名，如 `/injections`） |
...
| `/context`（原 `/usage`，2026-08-21 用户裁决更名——CC 语义） | kimi(/usage)/CC(/context) | — | 累计 input/output/cacheRead/cacheWrite/reasoning tokens + contextWindow 百分比 | 面板（`InfoPanel`） | ✅ sessionProjections.snapshot 读 tokenUsage/contextPressure 投影（§3.2）；`usage.ts` 纯折 assistant/* usage 兜底（replace-per-step 语义） | ✅ S25（2026-08-21） | 投影折整条持久日志，跨 resume 重放正确；Blue 不设累计器 |
