...
> Interaction supersession (2026-09-01): 本文中的 `Alt+S` 和 model 底部 thinking tab 是 S23 落地时的历史记录。`0.1.2-alpha.1` 已以显式 `Use for this session` action 取代该绑定；model 只保留 provider tab，进入列表后用 Up/Down 选模型、Left/Right 调 thinking。历史正文不是当前键位说明。
...
| `/preset [name]` | Blue 原创（D33） | 无参=列表；补全=`ctx.agentPresets.list()` | 列出/切换 agent 组合预设（standard/minimal/ptc/cordis/blue-cordis）；空会话限定 | BlueSelect 面板（mountEditorReplacement，/sessions 模式） | ⚠️ `ctx.agentPresets.list/recompose` **不在 dsh-base**（仅 web-app bundle）——Blue bundle patch 加 `agent-presets` 行 + 包依赖，直接启用上游 shipped/user roots，并只叠加唯一 Blue root `blue-cordis`；sessionBlank 守卫 + `agent-preset/selected` 事件配对（进程内直调无 wire 层强制） | ✅ S28（2026-08-21，SelectListPanel + `/preset <id>` 重派发写路径；薄宿主迁移 D37 使替换语义为真；2026-08-31 alpha.2 改为上游 shipped roster） | UI 能力探测走投影/键缺失（预设间能力面差异大：minimal 无 plan mode/compaction）；不提供旧 `code` alias；自定义 permission preset 注册 API 无（组合 YAML 固定），残余缝不请求 |
