...
| 命令 | kimi | pi | CC | Codex | Blue 现状/去向 |
|---|---|---|---|---|---|
| `/preset [name]` | — | — | — | — | **S28** 待建（D33：agent 组合预设切换——`ctx.agentPresets.list/recompose`，空会话限定 + `agent-preset/selected` 事件配对；该插件**不在 dsh-base**（仅 web-app bundle），Blue bundle 加行+带依赖） |
...
**Blue 侧残余**：命令零实现（随插件到达，/help 自动枚举）；**plan-review 专用呈现 ✅ S24b 落地（2026-08-21）**：`interaction/plan-review-panel.ts`——questions-plugin 对单问 `intent.kind === 'plan-review'` 分流（配对畸形/未来 intent kind/多问题批回退通用问卷），计划体（question.detail）经 `createMarkdown` 入 10 行滚动窗（pgup/pgdn + showing 尾行），其下为共享 kimi 分段 chrome 的**三按钮行**（dogfood 轮 1 裁决，撤销初版双选行 + 内联反馈编辑器——编辑框嵌面板观感差）：Approve（`intent.approve` 命名，绝不硬编码）、Reject（以另一选项标签作答，模型本轮内收到"chose to keep planning"）、Revise in chat（走 ASK_CANCELLED dismissal——本轮停止、plan 模式保留、模型等用户下一条消息）；←/→/↑/↓ 移动焦点 + 焦点描述行，Esc = Revise in chat。**随期勘误**：provider dismissal 码原为 Blue 自造 `ASK_DISMISSED`，而 dsh-plan-mode 只 catch `ASK_CANCELLED`（dsh-host-apiproxy 同码拒 dismissal）——此前 Esc 泄漏原始 rethrow 给模型，全局改 `ASK_CANCELLED` 后模型收到 crafted "user dismissed the plan review to speak instead; stay in plan mode" 消息（e2e 断言下请求消息含该文案）。
